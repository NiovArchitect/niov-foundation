...
- **ADR-0048** — Foundation/COSMP Personalization-Orchestration Substrate (Phase 3 Sub-Arc 3 register substantively; canonicalizes Foundation-owned governed personalization-orchestration substrate per Founder directive: the Foundation/COSMP constructs the governed working set BEFORE the LLM sees context; the LLM must never decide what memory it is allowed to see; apps compose UX; Otzar consumes the working set; agents reason over the working set; permissions/wallet-boundaries/clearance/audit/capsule-state remain Foundation-owned; personalization is dynamic (changes by moment, location, role, task, permission state) and permission-aware but not permission-fragile; 11 Q-PERS sub-decisions / locks at `[COSMP-PERSONALIZATION-ORCHESTRATION-QLOCK]` — Q-PERS-α α-1 docs-only ADR first; Q-PERS-β β-1 Foundation owns governed working-set construction (apps compose UX per β-3); Q-PERS-γ γ-1 required/recommended/optional/denied 4-tier permission matrix; Q-PERS-δ δ-3 hybrid high-level buildPersonalizedWorkingSet + COSMP primitives; Q-PERS-ε ε-1 integration-test simulation harness 10 scenarios before demo/UI; Q-PERS-ζ ζ-1 location/time first-class permissioned moment context; Q-PERS-η η-1 stable/dynamic/ephemeral personalization-capsule taxonomy; Q-PERS-θ θ-2 NEW audit literals proposed (WORKING_SET_BUILT + CONTEXT_USED_MANIFEST_RECORDED + PERSONALIZATION_DEGRADED + CROSS_ENTITY_CONTEXT_REQUESTED + PERSONALIZATION_SIGNAL_RECORDED; implementation deferred clean-transition); Q-PERS-ι ι-1 local/domain-scoped working-set builder with caching (deeper optimization measure-first ι-3); Q-PERS-κ κ-1 TurboQuant future research arc only; Q-PERS-λ λ-1 privacy-policy implications in ADR; **substrate-state observations**: COE assembleContext (`coe.service.ts:172`) is the LIVE governed 7-step working-set constructor (Founder core principle already implemented — LLM does not decide what it sees; arc EXTENDS it); EntityProfile.timezone + name + job_title LIVE (preferred_name/pronouns/locale GREENFIELD); OrgSettings enterprise governance defaults LIVE; TwinConfig role+autonomy LIVE; Permission.conditions JSON consent-extensible; Otzar priming moment-aware time precedent; degraded-mode app-layer patterns exist (not Foundation-canonical); CapsuleType taxonomy (PREFERENCE + DEVICE_DATA + SESSION_LEARNING + BEHAVIORAL_PATTERN + IDENTITY + DOMAIN_KNOWLEDGE + FOUNDATIONAL) partially canonical; missing GREENFIELD/PARTIAL = moment-context resolver + 4-tier permission envelope + Foundation-tier degraded contract + cross-context scoped-authorization workflow + high-level buildPersonalizedWorkingSet API + personalization-signal recording with provenance; RULE 21 Hawkseye research arc embedded (context engineering + agent memory: OpenAI Cookbook + Mem0 + Supermemory + Taskade; permission-aware + privacy-preserving personalization: Puda arXiv:2602.08268 + ACM 2025 + VERTU + SparkCo; enterprise governance + orchestration + MCP: AetherLink + OneReach + Atlan + FifthRow; confused-deputy orchestration extension of ADR-0046 §RS-G6-2; TurboQuant Google DeepMind ICLR 2026; 15+ sources retrieved 2026-05-19); 12-row adversarial threat model TP1-TP12 (net verdict: no code-tier vulnerability in existing governed-retrieval substrate; HIGH-severity TP1 personal→enterprise leak + TP2 enterprise→personal leak + TP3 permission-denied hallucinated context are about NEW moment-context + degraded + cross-context surfaces canonicalized PERS.2-PERS.6); cites RULE 0 + RULE 10 + RULE 11 + RULE 12 + RULE 13 + RULE 20 + RULE 21 + ADR-0001 (personal/enterprise DMW boundary) + ADR-0002 + ADR-0011 §Amendment + ADR-0016 (measure-first TurboQuant deferral) + ADR-0020 + ADR-0022 (COE scoring) + ADR-0033 §Decision 7 (cross-language; BEAM locality) + ADR-0036 (per-request indexed point-lookup; permission-envelope pattern) + ADR-0037 (jurisdiction; TP8) + ADR-0038 + ADR-0039 (DMWWorker + WalletCache caching precedent) + ADR-0042 (proposeMemoryUpdate + clean-transition) + ADR-0043 (embedding-PII caution; G3.9 privacy proofs) + ADR-0044 (dynamic decay) + ADR-0045 (stale-moment TP7) + ADR-0046 (dual-context + confused-deputy mitigations) + ADR-0047 (hardening) + ADR-0041 (Sub-arc 2 closed parent) + Patent US 12,517,919 + US 12,164,537 + US 12,399,904; Phase 3 Sub-Arc 3 IN FLIGHT throughout PERS.1-PERS.6; Sub-arc 2 + all per-gap ADRs preserved Accepted/CLOSED; Phase 3 global status NOT flipped (separate Founder QLOCK required); 6-sub-phase lineage (PERS.1 this commit docs-only ADR + PERS.2 permission-envelope + moment-context forward-substrate + PERS.3 buildPersonalizedWorkingSet API forward-substrate + PERS.4 degraded-mode contract forward-substrate + PERS.5 synthetic DMW simulation forward-substrate + PERS.6 closure cascade forward-substrate); Status: Accepted 2026-05-20 at PERS.6 `[COSMP-PERSONALIZATION-CLOSURE]` (originally Proposed 2026-05-19 at PERS.1 `[COSMP-PERSONALIZATION-ADR]`; flipped Proposed → Accepted at the PERS.6 closure cascade per RULE 20 Founder authorization); **Sub-Arc 3 personalization-orchestration is CLOSED 2026-05-20** (Phase 3 global status NOT flipped — separate Founder QLOCK required)
- **ADR-0049** — Government-Grade Hardening and Gap-Closure Program for Foundation/COSMP (GOVSEC umbrella ADR; the master government-grade gap-closure arc — **not security-only and not compliance theater** — spanning security, privacy, auditability, identity/session, AI/agent abuse, tenant isolation, supply chain, incident response, cryptography, operational resilience, correctness, scalability, and optimization-as-readiness; Founder doctrine: deeper than FedRAMP paperwork + no security theater + every meaningful gap owned + optimization counts wherever it affects security/scale/latency/correctness/resilience; GOVSEC.1 docs-only per Founder Q-GOVSEC-α α-1 + β-1 + γ-2 + η-3 reserved for code phases + θ-3 LOCKS at `[GOVSEC-GOVERNMENT-GRADE-HARDENING-G1-EXECUTE-VERIFY-AUTH]`; standards basis retrieved 2026-05-20: FedRAMP 20x + NIST SP 800-53 Rev.5 AC/AU/IA/SC/SI/IR/SR + NIST SP 800-63-4 AAL2/AAL3 + NIST AI RMF/GenAI + NIST PQC (symmetric-only core already PQC-resilient) + CISA ZTMM v2.0/Secure-by-Design + OWASP API Top 10 2023 + OWASP LLM Top 10 2025 + SLSA v1.0/SBOM + SOC 2 CC; companion `docs/reference/govsec-control-matrix.md` (standards→named-repo-surface matrix + full threat model A–L + Master Gap-Closure Register every gap owned by gap-id + Optimization/Resilience Register GAP-O1..O9 + phase-ownership/test-strategy/closure-criteria matrices); 7 RULE 13 blind spots resolved (BS1 ADMIN_ACTION emitted widely not-a-gap; BS2 SESSION_EXPIRED/REVOKED unemitted → GAP-G1; BS3 AI-grantor/SESSION_ONLY → GAP-D1/D2; BS4 hive/dept filtering absent → GAP-F1; BS5 assembleContext cited by symbol; BS6 synthetic-DMW S7 twin-denial → narrowed GAP-F3; BS7 detectOperation null pass-through → GAP-B1); 10-phase decomposition (GOVSEC.1 docs-only; GOVSEC.2 audit/evidence amends ADR-0002; GOVSEC.3 auth/session 800-63-4; GOVSEC.4 gateway/bot-swarm/adversarial-load; GOVSEC.5 admin/break-glass/dual-control amends ADR-0026/0027; GOVSEC.6 AI/agent abuse + confused-deputy amends ADR-0046; GOVSEC.7 tenant isolation/dept filtering amends ADR-0037/0041; GOVSEC.8 supply-chain/CI/SBOM/provenance SLSA L2→L3; GOVSEC.9 crypto agility/key rotation/FIPS-PQC amends ADR-0019; GOVSEC.10 incident response/monitoring/closure cascade); GOVSEC.2–10 forward-substrate each gated by a separate Founder QLOCK; cites RULE 0 + RULE 4 + RULE 10 + RULE 11 + RULE 12 + RULE 13 + RULE 18 + RULE 20 + RULE 21 + ADR-0002 + ADR-0019 + ADR-0022 + ADR-0026 + ADR-0027 + ADR-0036 + ADR-0037 + ADR-0041 + ADR-0046 + ADR-0047 + ADR-0048 + `docs/reference/govsec-control-matrix.md`; Status: Proposed 2026-05-20 at GOVSEC.1 `[GOVSEC-GOVERNMENT-GRADE-HARDENING]`
- **ADR-0050** — GOVSEC.5 Break-Glass / Time-Boxed Audit (GOVSEC.5 design-first ADR for GAP-K1; break-glass is greenfield, **no code in the ADR phase**; locks the design: a **time-boxed emergency grant** — not a permanent role, not a general bypass, **not a self-approval** — invoked with explicit justification, scoped to a specific privileged action/route, **mandatory `valid_until`** (no perpetual access); initial scope = the 4 dual-control `PRIVILEGED_ENDPOINTS` routes (broader org-admin set is a separate follow-on); normal dual-control remains the default and the GAP-C1 self-approval guard (`caller === source_entity_id` forbidden) stays intact; chosen relationship = authorized invoke + short time-box + **mandatory post-hoc two-person audit/review** (or second-actor-before-activation, EXECUTE-QLOCK-selected); future append-only audit literals (`BREAK_GLASS_INVOKED`/`USED`/`EXPIRED`/`REVIEWED`) in `audit.ts` with **no ADR-0002 amendment** for additive literals (G4-B1 / Sub-box-3 / ADR-0042 precedent); future `BreakGlassGrant`-style table mirroring `LawfulBasis` (mandatory `valid_until`) — `EscalationRequest.expires_at` is insufficient (approval-validity, not an access grant); RULE 21 research arc embedded (NIST SP 800-53 AC-3/AC-6/AU-6/AU-12 + NIST SP 800-207 ZTA + JIT access + break-glass PAM/HIPAA best practice + SOC 2 CC6 / ISO 27001 A.5.18); **GAP-K1 = designed/not-closed; GOVSEC.5 remains OPEN** (code + audit-completeness tests required to close); GAP-O7 open; D2-C / GOVSEC.7 deferred; cites RULE 0/11/12/13/20/21 + ADR-0049 + ADR-0026 + ADR-0036 + ADR-0002; **Status: Accepted 2026-05-22** (Proposed 2026-05-22 at `[GOVSEC-GOVERNMENT-GRADE-HARDENING-GOVSEC5-BREAK-GLASS-TIMEBOXED-AUDIT-ADR-AUTH]` → Accepted 2026-05-22 at the BG.3 closure cascade `[GOVSEC-GOVERNMENT-GRADE-HARDENING-GOVSEC5-BREAK-GLASS-BG3-CLOSURE-EXECUTE-VERIFY-AUTH]`; **GAP-K1 CLOSED** across ADR-0050 design + BG.1 substrate `2f487fc` + BG.2 live integration `f5f0256` (green CI run `26297686163`); accepted for break-glass / time-boxed audit only — **GOVSEC.5 remains OPEN** pending the broader org-admin `requireAdminCapability` route-set throttle follow-on + GAP-K2 least-privilege capability review))
- **ADR-0051** — Otzar Chat Transparency and COE-Governed Retrieval Surfacing (Wave 1 Otzar enterprise-experience contract ADR; surfaces governed context metadata `COE.assembleContext` already produces — items loaded / skipped-low-relevance / skipped-budget + an `access_limited` boolean + per-item provenance from already-permitted context items — via an **additive** extension of `conductSession` / `ConductSessionSuccess` (`apps/api/src/services/otzar/otzar.service.ts`); `context_used` + `tokens_consumed` + `response` + `conversation_id` unchanged (backward-compatible); two NEW OPTIONAL fields `transparency` + `context_provenance`; **no parallel `similarity.service` path, no COE bypass** (similarity deferred to a future Founder-authorized ADR-0022 amendment per ADR-0043 §Q-G3.6-ε); **no `/otzar/context/ingest` route, no new CapsuleType/source enum** (ingestion stays on existing `POST /cosmp/capsule` per ADR-0021/ADR-0042); transparency is a read-only projection of already-audited COE reads → **no new audit literal, no ADR-0002 amendment** (RULE 4 via existing `CAPSULE_CONTENT_READ` + `CONVERSATION_STARTED`); never exposes raw prompts/chain-of-thought/raw content/vectors/per-item scores/permission internals/raw denied counts/bridge IDs/capability flags/cross-tenant or unpermitted-teammate data; customer-facing copy avoids substrate jargon and never shows `context_id` prominently; Foundation contract first, `otzar-control-tower` second; additive touch of closed Otzar work (RULE 1) + cross-repo wire-format change (RULE 21) explicitly Founder-authorized at `[OTZAR-WAVE-1-ADR-0051-WRITE-AND-ACCEPT-AUTH]`; cites RULE 0/1/4/6/9/13/20/21 + ADR-0048 + ADR-0043 + ADR-0022 + ADR-0021 + ADR-0042 + ADR-0002 + ADR-0035; **RULE 14 back-citations into cited ADRs pending a follow-on Founder authorization** (this auth enumerated the ADR file + catalog only; editing cited ADR bodies is RULE-20-restricted); **Status: Accepted 2026-05-26** by Founder for Wave 1)
- **ADR-0052** — Otzar Domain General Intelligence and Governed Synchronicity (doctrine ADR; canonicalizes Otzar's product/architecture truth so future build sessions do not drift — NO runtime behavior, NO endpoints, NO schema, NO code; long-form companion `docs/otzar/DOMAIN_GENERAL_INTELLIGENCE_DOCTRINE.md`. Otzar is the governed enterprise intelligence layer on top of Foundation — the operating layer for the Autonomous Enterprise — a bounded **Domain General Intelligence** system inside governed enterprise domains, NOT a generic chatbot/admin-console/workflow-automation/AI-harness/thin-frontend; canonical *"Otzar turns the enterprise itself into a governed collaborative intelligence layer"*. Locks: enterprise as active knowledge collaborator (not passive storage); RULE 0 sovereignty (no AI accesses human/entity/enterprise data without explicit revocable governed permission; raw unpermitted data never exposed); human↔AI Twin **scope parity** (same RBAC/ABAC boundary, project visibility, tenant boundaries, escalation limits, allowed actions; the Twin is a scoped extension, never above the human, never a spy/manager/uncontrolled actor); *"Otzar does not watch employees to judge them"* — permissioned work observation, NOT surveillance; **drift** prevention first-class; governed best-practice learning (permissioned/scoped/auditable/revocable; no raw-data leakage, no hidden evaluation); scoped Twin-to-Twin coordination exchanging only minimum relevant info; *"Otzar can proactively prepare, coordinate, and recommend inside scope; it executes sensitive actions only under permission, policy, or approval"* (no boundary-crossing/cross-tenant leakage/hidden autonomy); **Control Tower** = governance/executive-clarity layer (AI-generated what-happened/why/needs-approval/risk/recommended-action summaries), not a classic admin dashboard; reports *"generated from permissioned operational signals and routed according to hierarchy, scope, and relevance"* (not raw access; executives get summaries without raw unpermitted employee data); multi-tenant + dynamic governed grouping each with scoped intelligence; DGI is **bounded** (no global AGI; powerful because governed/scoped inside a domain); **The Governed Synchronicity Loop** (alias Scoped Execution Intelligence Loop): Observe → Understand → Align → Assist → Coordinate → Approve → Execute → Report → Learn → Improve; canonical 15-step build order + sequencing guards (governance before autonomy; web coherence before desktop; context policy before MCP/connectors; transcript ownership/retention/scope policy before raw transcripts; single Twin + scope + approvals before hives; real backend events before dashboards; never fake autonomy); Wave 1 (ADR-0051) is *"the first governed transparency surface in the larger Domain General Intelligence path"*; cites RULE 0 + RULE 1 + RULE 4 + RULE 9 + RULE 13 + RULE 20 + ADR-0051 + ADR-0048 + ADR-0002; Status: **Accepted 2026-05-27** at `[OTZAR-DGI-DOCTRINE-FOUNDATION-DOCS-EXECUTE-VERIFY-AUTH]`)
- **ADR-0053** — Otzar Employee AI Twin Role-Scope Profile and Drift-Prevention Foundations (Wave 2 contract ADR; **design-only — NO code, NO schema, NO endpoints, NO tests**; ADR-0052 build-order step 3. Locks: (1) **Wave 2A primary** — additively extend `GET /api/v1/otzar/my-twin` (`getMyTwin`) into a richer governed employee AI Twin **role-scope profile**, **self-scoped only** (own twin only; no other employees' scope, no cross-tenant data, no raw permission internals / bridge IDs / capability flags / raw clearance, no hidden evaluation, no surveillance framing); (2) **safe derived fields** — identity + role (`role_title`/`job_title`/`department`/`hierarchy_level` if safe / `is_admin_twin`) + scope summary (tenant/department labels, membership count, high-level **permission posture label** not raw RBAC/ABAC, approval/escalation posture label) + assistance (`autonomy_mode`/`swarm_enabled`/`role_template` status/`skills` if real) + governance (approver; threshold actions require permission/policy/approval; observation is permissioned work context, not surveillance) + continuity (recent conversation/correction/learning counts; **no raw transcript content**); (3) **derived-first** — NO `Project`/`Responsibility` models in 2A (derive from `EntityProfile`/`EntityMembership`/`TwinConfig`/`Permission` safe-labels/`MemoryCapsule` counts/`OtzarConversation` metadata; new models need a later ADR); (4) **Wave 2B conversation look-back** bounded to metadata+summary only (**no raw transcripts / message bodies / prompts / chain-of-thought / cross-tenant / unpermitted teammate data**); locks **`summary_capsule_id` on `OtzarConversation`** as the preferred future additive link (not implemented here); (5) **drift-prevention FOUNDATIONS only** — `CORRECTION` is the first human-alignment primitive (already prioritized in `conductSession` Layer 1); 2A may expose only correction/learning counts + an "alignment signals available" label (no scoring/policing/hidden evaluation); full **drift** detection (recurring `CORRECTION` → `IntelligencePattern` write-side, stale-context warnings, drift-signal contract, proactive suggestions) is Wave 3; (6) **Control Tower** consumption deferred (no fake dashboards); (7) sequencing — **Foundation role/scope profile contract first**, then Foundation tests, then Control Tower My Twin UI second, look-back third, drift after; (8) non-goals — no autonomy/execution-engine/MCP/raw-transcripts/listener-sidecar/hives/Agent-Playground/desktop/Twin-to-Twin/Control-Tower-reports/notifications/**employee surveillance**/productivity-policing/hidden-evaluation/cross-tenant-leakage/raw-unpermitted-data/fake-capability-claims; acceptance = additive/backward-compatible + safe self-scoped + tests proving no internals leak. cites RULE 0 + RULE 1 + RULE 4 + RULE 9 + RULE 13 + RULE 20 + RULE 21 + ADR-0052 + ADR-0051 + ADR-0048 + ADR-0002; Status: **Accepted 2026-05-27** at `[OTZAR-WAVE-2-ADR-0053-EMPLOYEE-TWIN-ROLE-SCOPE-PROFILE-WRITE-AND-ACCEPT-AUTH]`)
- **ADR-0054** — Otzar Conversation Look-back and Safe Continuity Surfacing (Wave 2B contract ADR; **design-only — no code/endpoints; no schema migration in this phase**; ADR-0052 build-order step 4. Locks: (1) NEW self-scoped detail endpoint **`GET /api/v1/otzar/conversations/:id`** (bearer + `read`; caller reads only conversations where `OtzarConversation.entity_id` == authenticated entity); (2) **minimal additive response** — conversation_id/twin_id/source_type/status/started_at/closed_at/message_count/`summary`/`topics`/`summary_available`/`summary_capsule_id`/`detail_availability`/`transparency_available`/`continuity_note`; (3) `detail_availability` enum **`SUMMARY_AVAILABLE` | `NO_SUMMARY_YET` | `ACTIVE_NOT_CLOSED`**; (4) `summary` derived ONLY from the linked `CONVERSATION_LEARNING` capsule `payload_summary`, `topics` only from its `topic_tags`; pre-existing convos may have `summary_capsule_id = null` — **no backfill**; (5) **additive schema** — add nullable **`summary_capsule_id String? @db.Uuid`** to `OtzarConversation` (implements ADR-0053 §4 preferred link; retires fragile `storage_location` string-parse / D-OTZ-3); (6) `closeConversation` sets `summary_capsule_id` to the created CONVERSATION_LEARNING capsule id; (7) safe boundary — **no raw transcripts/messages/prompts/chain-of-thought/raw context/vectors/permission internals/bridge IDs/capability flags/cross-tenant data/fake retrospective transparency/fake corrections/fake continuity**; `summary` is a close summary, NOT a transcript; (8) honest deferred — `transparency_available` **always false** (ADR-0051 transparency is live **response-only**, not persisted); `continuity_note` states per-conversation correction/transparency signals are not retained; `corrections_count`/`context_provenance`/`access_limited` excluded; **do not fabricate** for past conversations; (9) access — cross-caller → `NOT_CONVERSATION_OWNER`, unknown id → `CONVERSATION_NOT_FOUND`, reuse existing `OtzarFailure` codes, **no new audit literal**; (10) migration via `db:push:test` per ADR-0025 (never bare `prisma db push`); (11) test reqs incl. wire-level no-leak + self-scope (repo-approved vitest tier configs, not bare vitest); (12) order — ADR → schema → closeConversation link → mapper → `getConversationDetail` service → route → tests → merge Foundation first → Control Tower later; (13)/(14) Control Tower detail UI later (metadata + summary + topics + honest "not retained" note; no raw transcripts); non-goals — raw transcript persistence/UI, listeners, MCP/connectors, hives, Agent Playground, autonomous execution, enterprise reporting, full audit viewer, dashboards-without-events, billing, per-conversation transparency persistence, conversation→correction linkage. cites RULE 0 + RULE 1 + RULE 4 + RULE 9 + RULE 13 + RULE 20 + RULE 21 + ADR-0053 + ADR-0051 + ADR-0052 + ADR-0002 (operational refs ADR-0042 + ADR-0025); Status: **Accepted 2026-05-27** at `[OTZAR-WAVE-2B-ADR-0054-CONVERSATION-LOOKBACK-WRITE-AND-ACCEPT-AUTH]`)
- **ADR-0055** — Otzar Correction Signals and Drift-Prevention Continuity (Wave 2C contract ADR; **design-only — no code/endpoints; no schema migration in this phase**; ADR-0052 build-order steps 5–6 + closes ADR-0054's explicitly-deferred `conversation→correction linkage` non-goal. Locks: (1) **Scope** — Wave 2C links correction submissions to conversation continuity and exposes per-conversation correction-signal counts + freshness through a safe self-scoped sub-resource; does NOT implement full drift detection / drift score / stale-context warnings / proactive suggestions / `IntelligencePattern` auto-write / org-wide aggregation / manager dashboards / employee scoring / autonomous execution / transcript replay / listeners / MCP / hives / Agent Playground / enterprise reporting / billing / full audit viewer; (2) **additive schema** — add nullable **`conversation_id String? @db.Uuid`** to `MemoryCapsule` + `@@index([wallet_id, capsule_type, conversation_id])`; no backfill; no new model; no new enum; no new audit literal; no FK constraint (service code owns existence + self-scope checks per Section 9); pre-existing `CORRECTION` capsules remain `conversation_id = null`; (3) **extend `POST /api/v1/otzar/correction`** — body adds OPTIONAL `conversation_id`; omitted → backward-compatible (persists null); provided → service validates ownership; unknown → `CONVERSATION_NOT_FOUND` (404); cross-caller → `NOT_CONVERSATION_OWNER` (403); on success the resulting `CORRECTION` capsule stores `conversation_id`; (4) **NEW** `GET /api/v1/otzar/conversations/:id/corrections` — bearer + `read`; self-scoped; cross-caller / unknown id same codes; reuse existing `OtzarFailure` codes; no new audit literal; (5) **safe response** — `{ ok, conversation_id, corrections_count, has_corrections, last_correction_at, drift_prevention_note, continuity_note }`; `corrections_count` is a real Prisma count of `capsule_type='CORRECTION'` + caller's wallet + `conversation_id = :id` + `deleted_at IS NULL`; `last_correction_at` is ISO `created_at` of the most-recent linked capsule or null; (6) **forbidden response fields** — raw correction `payload_summary`/`payload_content`; correction capsule IDs list; `target_capsule_id`; target capsule content; transcripts; raw messages/prompts/chain-of-thought/raw context; `storage_location`; `content_hash`; vectors; embeddings; permission-envelope internals; bridge IDs; capability flags; other users' / cross-tenant data; drift score; employee score; best-practice learned/applied status; manager visibility fields; org-wide aggregation; (7) **submitted vs learned/applied** — Wave 2C surfaces ONLY submitted/available correction signals (counts + last-seen); MUST NOT claim "best practice learned" / "behavior permanently fixed" / "drift prevented" / "AI fixed itself"; Layer 1 priority + `RELEVANCE_MAX` propagation are real but described as scoped correction priority within the caller's wallet, not autonomous learning; (8) **`ConversationDetailView` UNCHANGED** — ADR-0054 surface not modified; `transparency_available` stays false; corrections live on the new `/corrections` sub-resource; (9) **safe frontend language** locks allowed copy ("Correction signals" / "Corrections linked to this conversation" / "Not enough correction history yet" / "This does not expose raw messages." / "This is not an employee score." / "This is not a transcript.") and forbidden copy ("drift score" / "employee score" / "manager monitoring" / "surveillance" / "full history" / "message replay" / "best practice learned" / "AI fixed itself" / "autonomous drift prevention" / "all corrections across the org"); (10) access self-scoped throughout (no manager/admin/org-wide read path); (11) migration via `db:push:test` per ADR-0025 (never bare `prisma db push`); local DB reconciliation per ADR-0035 §38 / ADR-0047 PR.3 if needed; (12) cross-language boundary — Prisma owns `conversation_id`; Ecto schema NOT extended (ADR-0033 Q-G3-θ / G3.8 precedent); (13) test reqs — `processCorrection` persists `conversation_id` valid / backward-compat null / 404 unknown / 403 cross-caller; `GET …/corrections` real count + zero state + 403 cross-caller + 404 unknown + wire no-leak; ADR-0054 detail regression green; `getMyTwin` `recent_correction_count` regression green; Layer 1 fixture green; RULE 16 + 12-error typecheck baseline preserved; repo-approved vitest tier configs (ADR-0035 §37); (14) implementation order — ADR → schema → `POST /correction` extend → pure mapper → `getConversationCorrections` service → `GET …/corrections` route → tests → merge Foundation first → Control Tower later. cites RULE 0 + RULE 1 + RULE 4 + RULE 9 + RULE 13 + RULE 20 + RULE 21 + ADR-0054 + ADR-0053 + ADR-0052 + ADR-0051 + ADR-0025 + ADR-0002 (operational refs ADR-0033 + ADR-0035 + ADR-0042 + ADR-0047); Patent-implementation evidence — US 12,517,919 (COSMP) + US 12,164,537 (DMW) + US 12,399,904 (scoped wallet-bound continuity; no cross-tenant fusion); Status: **Accepted 2026-05-27** at `[OTZAR-WAVE-2C-ADR-0055-CORRECTION-CONVERSATION-LINKAGE-WRITE-AND-ACCEPT-AUTH]`)
