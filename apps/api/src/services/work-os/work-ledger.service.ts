...
// Deterministic risk detection over durable Work Ledger state. "Here is what may
// quietly hurt the organization if nobody acts." No AI guessing; no fake items.
...
const BLIND_SPOT_FEED_TYPES = ["TASK", "FOLLOW_UP", "APPROVAL", "BLOCKER", "DECISION"];
const STALE_MS = 48 * 60 * 60 * 1000;
const WEEK_MS = 7 * 24 * 60 * 60 * 1000;

// WHAT: typed risk feed (overdue / stale waiting-on / unresolved blocker /
//        no-next-action) from durable ledger entries. Scope mirrors getBlindSpots
//        (caller's own work unless manager → org-wide). Tenant-isolated.
export async function getBlindSpotFeed(args: {
  ...
  const rows = await prisma.workLedgerEntry.findMany({
    where: {
      ...scope,
      ledger_type: { in: BLIND_SPOT_FEED_TYPES },
      NOT: { status: { in: ["CANCELLED", "EXPIRED", "VERIFIED", "EXECUTED"] } },
    },
  ...
  const names = await resolveEntityNames(
    rows.flatMap((r) => [r.owner_entity_id, r.requester_entity_id]),
  );
  const items: BlindSpotFeedItem[] = [];
  const daysSince = (d: Date): number => Math.floor((now - d.getTime()) / (24 * 60 * 60 * 1000));
  for (const r of rows) {
    const ownerName = r.owner_entity_id !== null ? nameFrom(names, r.owner_entity_id) : null;
    const requesterName = r.requester_entity_id !== null ? nameFrom(names, r.requester_entity_id) : null;
    const base = {
      title: r.title,
    ...
    if (r.due_at !== null && r.due_at.getTime() < now) {
      ...
      candidates.push({
        ...
        type: "OVERDUE_WORK",
        summary: `Due ${overdueDays}d ago and still open.`,
        severity: overdueDays > 7 ? "HIGH" : "MEDIUM",
        recommended_action: `Nudge ${ownerLabel} or reset the due date.`,
        detection_rule: "active item with due_at in the past",
      });
    }
    ...
    if (r.ledger_type === "BLOCKER") {
      candidates.push({
        ...
        summary: `Open blocker (${base.age_days}d old).`,
        severity: "HIGH",
        recommended_action: "Resolve the blocker or escalate it.",
        detection_rule: "active BLOCKER ledger entry",
      });
    }
    ...
    if (
      ...
      candidates.push({
        ...
        summary: `${requesterName ?? "Someone"} is waiting on ${ownerLabel} — no movement in ${staleDays}d.`,
        severity: now - r.updated_at.getTime() > WEEK_MS ? "HIGH" : "MEDIUM",
        recommended_action: `Nudge ${ownerLabel} or re-scope the ask.`,
        detection_rule: "directional waiting-on with no update in 48h",
      });
    }
    ...
    if (r.owner_entity_id === null || r.next_action === null || r.next_action === "") {
      candidates.push({
        ...
        type: "NO_NEXT_ACTION",
        summary: r.owner_entity_id === null ? "No owner assigned." : "No next action set.",
        severity: r.owner_entity_id === null ? "HIGH" : "LOW",
        recommended_action: r.owner_entity_id === null ? "Assign an owner." : "Set a clear next action.",
