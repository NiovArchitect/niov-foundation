...
export async function queryOrgWork(args: OrgQueryArgs): Promise<OrgQueryOutcome> {
  ...
  if (scope === "self") {
    ...
    where.ledger_type = { notIn: ["ORG_SEEDING", "GOAL", "DOCUMENT_CONTEXT"] }; // seeds are admin org-seeding, not personal work
    ...
    where.ledger_type = { notIn: ["ORG_SEEDING", "GOAL", "DOCUMENT_CONTEXT"] };
    where.status = { notIn: [...CLOSED_STATUSES] };
