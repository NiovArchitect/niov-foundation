  ...
  "apps/api/src/routes/org.routes.ts": [
    {
      ...
      reason:
        "Prisma select inside admin route — column read, not response shape; mapper-tier projection strips it before response. Line shifted 898 → 1089 by the ADR-0057 §7 Operation E PUT /api/v1/org/action-policies insertion (+191 lines), then 1089 → 1175 by the ADR-0057 Wave 7 PR #49 retry_budget + attempt_timeout_ms_override admin write-path extension (+86 lines), then 1175 → 1176 by the Section 4 Wave 3 INVOKE_CONNECTOR ActionType allowlist insertion in the same file (+1 line), then 1176 → 1177 by the D6 starter-pilot activation runtime import addition (+1 line), then 1177 → 1180 by the D6 team archetype runtime import expansion + executeTeamActivationForCaller route addition (+3 lines for the import block growth), then 1180 → 1181 by the D6 business archetype runtime import expansion (+1 line for executeBusinessActivationForCaller), then 1181 → 1182 by the D6 enterprise archetype runtime import expansion (+1 line for executeEnterpriseActivationForCaller), then 1182 → 1200 by the D6 DUAL-CONTROL middleware wiring (requireDualControl + PRIVILEGED_ENDPOINTS imports + enterpriseActivationEndpoint registry lookup at the top of registerOrgRoutes; +18 lines), then 1200 → 1230 by the [FOUNDATION-CLOSE-DC-WIRING-DRIFT-ACTION-POLICIES] Section 10 hardening pass close — added actionPolicyEndpoint PRIVILEGED_ENDPOINTS.find lookup adjacent to enterpriseActivationEndpoint (+18 lines for the comment + lookup + throw-guard block) and the PUT /api/v1/org/action-policies preHandler conversion from single requireAdminCapability to [requireAdminCapability, requireDualControl(actionPolicyEndpoint)] tuple (+12 lines for the multi-line preHandler block expansion), then 1230 → 1232 by the [BILLING-TWIN-CREATION-ENTITLEMENT-METER] B5-α + B6-α wiring at twin creation — added assertEntitledForOrgSoftGate + recordUsageForOrg imports at the top of the import block (+2 lines) — same canonical code, same substrate justification.",
    },
  ],
