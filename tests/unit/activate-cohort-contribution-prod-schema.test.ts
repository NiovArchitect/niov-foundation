// FILE: tests/unit/activate-cohort-contribution-prod-schema.test.ts
// PURPOSE: Phase 1306-B — guard the safety invariants of the cohort-contribution
...
// CONNECTS TO: scripts/activate-cohort-contribution-prod-schema.ts
...
const SCRIPT = path.join(REPO_ROOT, "scripts/activate-cohort-contribution-prod-schema.ts");
const APPROVAL_PHRASE = "APPROVE COHORT PROD SCHEMA ACTIVATION — additive only";
const src = readFileSync(SCRIPT, "utf8");

describe("Phase 1306-B — cohort-contribution prod-activation safety invariants", () => {
  ...
  it("only creates the contribution enum + table + indexes", () => {
    expect(src).toContain('CREATE TYPE "CohortContributionStatus"');
    expect(src).toContain('CREATE TABLE IF NOT EXISTS "cohort_contributions"');
  ...
  it("is idempotent (IF NOT EXISTS + pg_type guard)", () => {
    ...
    expect(src).toContain("SELECT 1 FROM pg_type WHERE typname = 'CohortContributionStatus'");
  });
