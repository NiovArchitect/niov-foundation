...
- **Latest main HEAD:** `d5ea615` (Wave 9 vNext closeout docs LIVE 2026-05-31). **Wave 10 Section 2 Action read-surface integration LIVE 2026-05-31** at `otzar-control-tower` PR #8 `ade4981` (ZERO Foundation backend changes; CT cockpit consumes existing `GET /api/v1/actions/:id` per ADR-0057 §9 + §10 verbatim; 16 NEW lifecycle tests + 126/126 total CT tests passing).
- **Latest merged PR:** [#153](https://github.com/NiovArchitect/niov-foundation/pull/153) — Close out Section 5 Wave 9 vNext runtime + Wave 10 lockstep — ADR-0076 §4.2 + §5.2 vNext LIVE in Foundation + Control Tower.
