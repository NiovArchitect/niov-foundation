...
- **Latest main HEAD:** `ed99dbd` (PR #242 Enterprise Twin Runtime V1 read-only state projection LIVE 2026-06-03). **CT main HEAD:** `de77cdf` (CT PR #32 INVOKE_CONNECTOR CT surface LIVE 2026-06-02). **LEI sequence COMPLETE end-to-end** at the substantive runtime register — DOCTRINE round (PRs #222-231 ADR-0086 through ADR-0094) + SUBSTANTIVE RUNTIME round (PRs #232-242: B5-α + B6-α + Voice seam + PY4 + DMW DM1-A/B + DM2-A/B + DM3-A/B + Enterprise Twin V1). **ADR-0092 §4 ALL 3 CANDIDATES LIVE end-to-end** (Consent+Receipt + Scoped Voice Memory Gate + AI Teammate Delegation Frame). Section 6 AnalyticsService: 7 → **8 LIVE aggregates** (added ENTERPRISE_TWIN_STATE). 1656/1656 unit tier passing (+247 from session start; was 1409). 8 new audit literals added this round (ENTITLEMENT_CHECK_DENIED + USAGE_METER_RECORDED + CONSENT_GRANT_RECORDED + CONVERSATION_MEMORY_SCOPE_DECLARED + TEAM_DELEGATION_CREATED + SWARM_BOUNDARY_DECLARED — all append-only per ADR-0042 §Q-γ.1).
- **Latest merged PR:** [#242](https://github.com/NiovArchitect/niov-foundation/pull/242) — Enterprise Twin Runtime V1 read-only state projection (first Enterprise Twin slice; NEW ENTERPRISE_TWIN_STATE aggregate composing against 7 LIVE substrates; 5 closed-vocab posture labels; 15-field SAFE projection; 15 NEW unit tests).
- **Active branch / PR:** `foundation-closeout-lei-substantive-runtime-round` — docs-only closeout consolidating the substantive runtime round (11 PRs #232-242).
