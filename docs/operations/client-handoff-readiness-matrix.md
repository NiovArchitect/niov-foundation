...
| 1245 | Final enterprise demo path | **PROD** (docs; runs on existing substrate) | The canonical 28-step handoff demo recorded in enterprise-handoff-runbook.md §18 — deterministic and credential-free; each step names its proving substrate; steps touching the five pending tables run on the local/test DB until the schema push. |
