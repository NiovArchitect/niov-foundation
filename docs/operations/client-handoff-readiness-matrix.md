...
| 1250 | Governed transaction readiness | **PROD** (governance substrate; zero schema changes) + **DEMO_ONLY** (mock rail) | governed-transaction.service.ts + otzar-settlement.routes.ts: DMW actors (humans, AI Twins, AI Employees, devices, agents) propose mock transaction intents; the policy gate tiers by amount (micro ≤ $1, dual control ≥ $1,000) and actor class (AI/device/machine NEVER auto-approve); humans approve (self-approval forbidden, dual control = two distinct humans); the MOCK rail emits clearly-labeled proofs; the append-only audit chain IS the intent store (5 new TRANSACTION_* literals — event-sourced, immutable per ADR-0002, runs on the CURRENT production schema). Kill switch bites at settle time; tenants isolated; regulator evidence redacted. 10 unit + 9 integration tests + demo-walk step 29. Real rails: BLOCKED_BY_CREDENTIALS + EXPLICIT_AUTHORIZATION_REQUIRED. Real funds: NOT_AUTHORIZED. Private keys: NOT_HANDLED. |
...
| **SCHEMA LIVE (pushed 2026-06-12, Founder-approved, additive-only)** | Collaboration Workspaces + external stakeholders, Meeting capture, Voice capture tables, Observe captures, Onboarding persistence, Compliance share packages, Work Comms substrate (schema only; runtime slices follow separate authorization) — all moved into PROD/schema-live; the pending set is now EMPTY |
