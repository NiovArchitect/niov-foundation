...
MOD `connector.service.ts` — `ConnectorType` extended (7-type → 8-type registry); `CONNECTOR_REGISTRY.MICROSOFT_365_READ` frozen entry with `tenant_id` config field (Azure AD tenant identifier; analogous role to C3 `workspace_domain` or C4-A `cloud_id`); `getConnectorTypeDefinition` + `getConnectorProviderAsync` dispatch wired.

NEW `tests/unit/c5-microsoft-365-read-provider.test.ts` — 28 tests across registry extension + factory + fixture-mode success per operation + 5 payload validation (rejects `calendar.events.create` / `mail.messages.send` / `drive.items.upload` / `teams.list` as VALIDATION — Teams reads forward-substrate to a later C-slice) + 8 forced-failure fixture keys + environment gate + privacy invariant (no `Bearer` / no JWT prefix `eyJ...` / no `subject` / no `body` / no `attendee` / no `from` / no `recipient` / no `@` / no `outlook.com` / no `onmicrosoft.com` / no `filename` / no `displayName` in delivery_metadata; whitelist-asserts the exact metadata keys per operation).

MOD `tests/unit/connector-provider.test.ts` — frozen-anchor contract test updated for **8-type registry; the description note now reads "6/6 connector matrix at RUNTIME_READY"**.
...
**Out of scope at C5** (forward-substrate): writes (≥C6; `events.create` / `files.upload` / `messages.send`), OAuth refresh-token rotation (later C-slice), OneDrive content download (≥C5+), Outlook mail body read (≥C5+), Teams read (later C-slice; Microsoft Graph `/me/joinedTeams` + `/chats` surface), webhook / change-notification subscriptions (≥C7), SharePoint / OneNote / Planner / Bookings reads (later C-slice), Control Tower binding-creation UI for `MICROSOFT_365_READ` (separate CT slice; Section 4 Wave 2 admin routes already accept `type: "MICROSOFT_365_READ"` without modification).

**Section 4 graduation across all 6 ranked connectors — 6 of 6 at RUNTIME_READY or higher:**
...
**6 of 6 ranked connectors at RUNTIME_READY or higher.** The connector substrate matrix is now complete at runtime tier; the only remaining graduation is Microsoft 365 → OPERATING via the CT C5 admin path slice.

**Next slice candidates:**

1. **CT C5 Microsoft 365 admin path** — operator-visible `/connectors` registration for `MICROSOFT_365_READ` mirroring CT PR #22 Google Workspace shape; graduates Microsoft 365 `RUNTIME_READY` → `OPERATING (admin self-serve)`; closes the 6/6 connector matrix at OPERATING parity.
2. **D6-ENTERPRISE-AUDIT-ONLY-TABLES** — Section 10 hardening item 2 (BreakGlassGrant + LawfulBasisAttestation + BoardObserverScope tables).
3. **W4 Proposed Action substrate** — Classification A docs forward-substrate.
...
NEW `apps/api/src/services/connector/github-read.provider.ts` — `GitHubReadProvider` implementing `ConnectorProvider`. Three read operations: `user` (current authenticated GitHub user smoke probe; boolean `authenticated` + `active` only — never user id / login / email) + `repos.list` (repos accessible to the token; counts + has_next_page only — never repo names / owner logins / branch names) + `issues.search` (issues assigned to authenticated caller via `GET /search/issues?q=is:issue+assignee:@me`; state aggregates grouped from GitHub `state` + `state_reason` fields into 3 buckets: `open_count` + `closed_completed_count` + `closed_not_planned_count` — never issue identifiers / titles / bodies / assignee email / reporter login / comments). OAuth 2.0 access token OR Personal Access Token (PAT) via `binding.secret_ref` env-var-NAME per ADR-0019 + ADR-0024. Fixture-first: real GitHub API only when `GITHUB_USE_REAL=1` + `config.use_real=true` + `secret_ref` resolves; triple defensive gate.
...
MOD `connector.service.ts` — `ConnectorType` extended (6-type → 7-type registry); `CONNECTOR_REGISTRY.GITHUB_READ` frozen entry; `getConnectorTypeDefinition` + `getConnectorProviderAsync` dispatch wired.

NEW `tests/unit/c-github-read-provider.test.ts` — 28 tests across registry extension + factory + fixture-mode success per operation + payload validation (rejects `issues.create` / `repos.create` / `pulls.create` / `graphql` as VALIDATION) + 8 forced-failure fixture keys + environment gate + privacy invariant (no `Bearer` / no `ghp_` / no `github_pat_` / no `gho_` / no `ghs_` / no `title` / no `body` / no `assignee` / no `login` / no `repo_name` / no `@` in delivery_metadata; whitelist-asserts the exact metadata keys per operation including 3 state aggregate buckets for issues.search).

MOD `tests/unit/connector-provider.test.ts` — frozen-anchor contract test updated for 7-type registry.
...
**Out of scope at C-GitHub** (forward-substrate): writes (≥C6; `issues.create` / `repos.create` / `pulls.create`), GitHub App JWT exchange flow (forward-substrate; static admin-supplied access token only; installation-token rotation deferred), PR / commit / branch / file-content reads (later C-slice), webhook ingestion via `verifyInboundHmac` (≥C7), GraphQL v4 surface (forward-substrate; REST v3 only at C-GitHub), Control Tower binding-creation UI for `GITHUB_READ` (separate CT slice; Section 4 Wave 2 admin routes already accept `type: "GITHUB_READ"` without modification).
...
1. **CT C-GitHub admin path** — operator-visible `/connectors` registration for `GITHUB_READ` mirroring CT PR #29 Linear shape; graduates GitHub `RUNTIME_READY` → `OPERATING (admin self-serve)`.
2. **C5 Microsoft 365 read-first connector runtime** — broader Microsoft Graph surface (calendar + Drive + Mail + Teams metadata-only).
3. **D6-ENTERPRISE-AUDIT-ONLY-TABLES** — Section 10 hardening item 2 (BreakGlassGrant + LawfulBasisAttestation + BoardObserverScope tables).
