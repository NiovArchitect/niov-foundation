...
- **I** `[SEC-CONTRIBUTOR-GOVERNANCE]` ✅ — ADR-0027 ("Contributor Governance + AI-Alignment + Rule-Modification Authority") + RULE 20 (Rule-Modification Authority — only the patent-holder Founder may modify/add/remove RULES or ADRs) + `docs/contributing/onboarding-for-engineers.md` NEW (the human-engineer onboarding doc); cascade: 26→27 ADRs (README + CLAUDE.md §5) + 19→20 RULES (CLAUDE.md §3 + the preamble RULE-count reconciliation + `docs/contributing/onboarding.md`) + ADR-0020's "(cited from)" block + the `135fee0` H-hash backfill above (this commit; this entry's own hash cascaded into this list + `section-12-progress.md` at sub-phase J per the post-commit-hash discipline)
