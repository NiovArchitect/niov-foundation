...
- Forward: ADR-0026 at sub-phase H (the full dual-control middleware + privileged-endpoint-registry + per-route-binding-discipline pattern; will cite this canonical record); ADR-0028 at sub-phase J (the Elixir/BEAM coordination-layer forward-substrate; the 6 BEAM-compatibility patterns this record documents at §5)
...
Events 1-3 + 4 (delegated) form the happy-path sequence; events 1-2 + 5 (denied) form the rejection sequence. All events are written inside the preHandler's own transactional scope per RULE 4 (the audit write and the verification outcome commit or roll back together — if the audit write fails, the verification fails closed and the handler is not delegated).
...
- **C** `[SEC-DUAL-CONTROL-CANONICAL-RECORD]` — this document
- **D** `[SEC-PRIVILEGED-REGISTRY]` — `apps/api/src/security/privileged-endpoints.ts` (the runtime registry; LIVE entries only — the 2 Category (1) operations; type-safe; unit tests)
- **E** `[SEC-DUAL-CONTROL-MIDDLEWARE]` — `apps/api/src/middleware/dual-control.middleware.ts` (consumes the registry; verifies an `APPROVED` `EscalationRequest`; writes the Zone U1 audit-event sequence per §4; adopts the 6 BEAM-compatibility patterns per §5; unit tests)
- **F** `[SEC-DUAL-CONTROL-BINDING-CONFIG]` — bind `requireDualControl` to `PATCH /api/v1/platform/monetization/config`; integration-tier test per ADR-0011 (full Zone U1 audit-chain end-to-end)
- **G** `[SEC-DUAL-CONTROL-BINDING-ORGS]` — bind `requireDualControl` to `POST /api/v1/platform/orgs`; integration-tier test per ADR-0011
- **H** `[SEC-DUAL-CONTROL-ADR]` — ADR-0026 ("Dual-Control Middleware Pattern + Privileged Endpoint Registry Runtime-Substrate + Per-Route Binding Discipline"); cites this canonical record; updates `section-12-progress.md` Sub-box 2 status + the CLAUDE.md §5 jump-table (25→26 ADRs) + the README ADR catalog (25→26)
- **I** `[SEC-CONTRIBUTOR-GOVERNANCE]` — ADR-0027 (Contributor Governance + AI-Alignment Discipline) + RULE 20 (Rule-Modification Authority) + `docs/contributing/onboarding-for-engineers.md` NEW (→ 27 ADRs)
- **J** `[SEC-BEAM-FORWARD-SUBSTRATE]` — ADR-0028 ("Forward-Substrate: Elixir/BEAM Coordination Layer for Capsule Supervision + OtzarComm + DBGI Integration"; explicit Elixir, not raw Erlang; capsule supervision trees; OtzarComm messaging at scale; DBGI as supervised process groups; multi-region BEAM clustering; migration triggers — DMW capsule count >1M, OtzarComm projected >10M-100M daily, multi-region deployment; the 6 BEAM-compatibility patterns; hybrid coexistence — Fastify+TypeScript API + Elixir coordination + Python intelligence + Postgres storage) (→ 28 ADRs)
...
- Forward: ADR-0026 (sub-phase H — the full dual-control middleware + privileged-endpoint-registry + per-route-binding-discipline pattern; will cite this canonical record); ADR-0028 (sub-phase J — the Elixir/BEAM coordination-layer forward-substrate)
...
- Forward: ADR-0026 at sub-phase H will cite this canonical record (the ADR bundles the full dual-control pattern; this record is its operational companion). `docs/architecture/dynamic-flow-architecture.md` RAA 12.7 §2.5 is referenced here as a foundational-canonicalization citation (the Zone U1-U4 trust-root layer) — no reciprocal back-cite at sub-phase C (RAA 12.7 §2.5 is referenced by-citation across the codebase without reciprocity; the RULE-14 foundational-vs-novel-citation distinction is forward-queued for a CLAUDE.md amendment, likely at sub-phase I alongside RULE 20).
