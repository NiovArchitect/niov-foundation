...
- **Section 5 Wave 7 best-path recommender contract ADR** — per ADR-0065 §7 + ADR-0073 §20. Design-only ADR locking how the Wave 6 `ComparisonResponse` composes into a recommended candidate + reasons + evidence + governance findings. NO autonomous execution; NO numeric scoring beyond closed-vocab confidence labels; NO bypass of Section 2 Action runtime for any transition (Wave 8 forward-substrate). Consumes Wave 6 Option A output as input.
