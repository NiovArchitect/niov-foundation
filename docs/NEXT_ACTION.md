...
- **Foundation main HEAD:** `ac33560` (Foundation closeout PR #215 for Linear LIVE; **C-GitHub Foundation read-first connector runtime in flight**). **CT main HEAD:** `caad758` (CT PR #29 — Linear admin path LIVE). Sections 5 + 7 + 9 Approvals + 9 Policies remain PRODUCTION-GRADE COMPLETE. **C-GitHub LANDING per `[FOUNDER-AUTONOMOUS-PRODUCTION-GO-LIVE-AUTH]`** — Classification E (runtime; 5th real vendor connector). NEW `GitHubReadProvider` implementing 3 REST read operations (`user` smoke probe via GET /user + `repos.list` via GET /user/repos + `issues.search` via GET /search/issues with `is:issue+assignee:@me` qualifier). MOD `connector.service.ts` — `ConnectorType` union 6 → 7; `CONNECTOR_REGISTRY.GITHUB_READ` frozen entry; dispatch wired. Fixture-first defensive triple gate (`GITHUB_USE_REAL=1` + `config.use_real=true` + `secret_ref` resolves). Bearer auth (OAuth 2.0 OR PAT); `X-GitHub-Api-Version: 2022-11-28` pinned. State aggregates from GitHub `state` + `state_reason` fields: `open_count` + `closed_completed_count` + `closed_not_planned_count`. 401 + 403 both collapse to AUTH. Privacy invariant: delivery_metadata carries counts only; NEVER Bearer / `ghp_` / `github_pat_` / `gho_` / `ghs_` / repo names / issue titles / bodies / assignee email / user login. RULE 21 research arc embedded. 28 NEW unit tests + frozen-anchor contract updated 6-type → 7-type. Unit suite 1353 → 1381; typecheck 4-error baseline preserved. **Section 4 GitHub `RECOMMENDATION_READY` → `RUNTIME_READY`** — 5 of 6 ranked connectors now at RUNTIME_READY or higher; only Microsoft 365 remains RECOMMENDATION_READY. **Next recommended:** **CT C-GitHub admin path** (mirror CT PR #29 Linear; graduates GitHub OPERATING) OR **C5 Microsoft 365 read-first runtime** via Microsoft Graph OR **D6-ENTERPRISE-AUDIT-ONLY-TABLES** OR **W4 Proposed Action substrate**.
- **Latest merged PR (Foundation):** [#215](https://github.com/NiovArchitect/niov-foundation/pull/215) — Foundation closeout for CT C4-B Linear admin path (docs-only; Project/Engineering family 2/2 OPERATING graduation recorded). **Latest CT PR:** [#29](https://github.com/NiovArchitect/otzar-control-tower/pull/29) — CT C4-B Linear admin path.
...
- **Active branch / PR (Foundation):** `foundation-c-github-read-connector-runtime` — Classification E fifth-real-vendor connector mirroring C4-A Jira REST pattern verbatim. NEW GitHubReadProvider + CONNECTOR_REGISTRY extension + 28 NEW unit tests + frozen-anchor contract refresh + Section 4 doc + NEXT_ACTION refresh.
