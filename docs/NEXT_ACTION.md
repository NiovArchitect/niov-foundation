...
- **Main HEAD:** `87bff72` (Section 2 Action read-surface integration for Wave 10 closeout docs LIVE 2026-05-31 PR #154). CT main HEAD: `ade4981` (Wave 10 ActionLifecyclePanel LIVE). Section 5 Agent Playground LIVE end-to-end with ADR-0076 vNext runtime + Wave 10 cockpit + Section 2 lifecycle read-surface + ADR-0077 §8.4 three-state-lifecycle honesty canonical across both repos.
- **Latest merged PR (Foundation):** [#154](https://github.com/NiovArchitect/niov-foundation/pull/154) — Close out Section 2 Action read-surface integration for Wave 10 — Foundation docs refresh for ADR-0077 §8.4 three-state-lifecycle honesty CANONICAL.
- **Active branch / PR:** `adr-0078-conversation-context-signals-substrate` (ADR-0078 design-only — four-layer conversation substrate; transcripts as source-of-truth + signals as safe projection + capture-eligibility/work-relevance filtering).
...
- **ADR-0078 LANDED 2026-05-31** at `[FOUNDER-CONVERSATION-CONTEXT-SIGNALS-SUBSTRATE-ADR-AUTH]` — design-only four-layer conversation substrate (Layer 1 Raw Transcript Source-of-Truth + Layer 2 Scoped Reasoning + Layer 3 `conversation_context_signals[]` Safe Projection + Layer 4 Permissioned Evidence Drilldown) + §6C capture-eligibility/work-relevance filtering layer (bias toward privacy; sensitive-personal BLOCKED from Agent Playground entirely) + §6B four canonical access tiers. **Next forward-substrate (each requires separate Founder authorization)**: ADR-0079 Transcript Substrate Policy (load-bearing prerequisite for ADR-0078 Stage 1+; MUST cover §6 12 prerequisites + §6A 19 governance capabilities + §6B 4 access tiers + §6C work-relevance filtering); ADR-0078 Stage 1 implementation (Layer 1 schema + Layer 3 helper + Layer 4 read service); ADR-0078 Stage 2 approved-source projection (CORRECTION_SIGNAL/ACTION_HISTORY/HIVE_CONTEXT/MANUAL_USER_INPUT sources LIVE today); Wave 9 Option C BEAM-orchestrated (ADR-0028 + ADR-0069 §6); Wave 10 hierarchy substrate; Wave 10 organizational graph.
