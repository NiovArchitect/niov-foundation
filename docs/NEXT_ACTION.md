...
- **Main HEAD:** TBD post-merge (ADR-0078 Stage 2 approved-source projection LIVE in branch `adr-0078-stage-2-approved-source-projection`). Prior main: `70a66cc` (ADR-0079 design-only Policy ADR LANDED 2026-05-31 PR #156). CT main HEAD: `ade4981` (Wave 10 ActionLifecyclePanel LIVE). Section 5 Agent Playground LIVE end-to-end + ADR-0078 four-layer substrate doctrine LIVE design-only + Stage 2 approved-source projection LIVE for Wave 7 + Wave 9 across Foundation.
- **Latest merged PR (Foundation):** [#156](https://github.com/NiovArchitect/niov-foundation/pull/156) — Add ADR-0079 design-only Policy ADR — Transcript Substrate Policy for Conversation Context Signals (load-bearing prerequisite for ADR-0078 Stage 1+; 33 sections + 9 service-tier gates).
- **Active branch / PR:** `adr-0078-stage-2-approved-source-projection` — ADR-0078 Stage 2 — additive `conversation_context_signals: readonly ConversationContextSignal[]` sidecar on Wave 7 `RecommendBestPathSuccess` + Wave 9 `EnterpriseDecisionPosture` per ADR-0078 §8 / §9. ZERO new audit literal, ZERO schema, ZERO new route, ZERO Layer 1 transcript ingest, ZERO Control Tower code. Approved sources LIVE: CORRECTION_SIGNAL (ADR-0055+0058) + ACTION_HISTORY (ADR-0057 SafeActionView) + MANUAL_USER_INPUT (ADR-0065 structural presence only). HIVE_CONTEXT preserved at enum register / zero-output at Stage 2. ADR-0079 §27 enforced by construction; §6C.12 8 additive fields exhaustive on every emitted signal; bounded ≤ 8. 17 NEW unit tests + 6 NEW Wave 7 integration tests + 7 NEW Wave 9 integration tests (Wave 7 40 → 46 / Wave 9 51 → 58 / Wave 8 43/43 regression preserved).
...
- **ADR-0078 Stage 2 LIVE 2026-06-01** at `[ADR-0078-STAGE-2-APPROVED-SOURCE-PROJECTION]` (this branch) — Foundation Wave 7 + Wave 9 responses carry additive `conversation_context_signals: readonly ConversationContextSignal[]` sidecars from already-LIVE approved sources (CORRECTION_SIGNAL / ACTION_HISTORY / MANUAL_USER_INPUT; HIVE_CONTEXT enum-preserved zero-output). NO raw transcript ingest, NO Layer 1 schema, NO Layer 4 drilldown, NO Control Tower code, NO new audit literal, NO schema migration. ADR-0079 §27 enforcement applied by construction. §6C.12 8 additive fields mandatory on every emitted signal. Bounded ≤ 8. **Recommended next slice: Control Tower consumption of `conversation_context_signals[]` on `/agent-playground`** — replace ADR-0077 §8.2 "not available" placeholder with safe Layer 3 signals only (no transcript drilldown; no quote/excerpt; no manager surveillance; no employee scoring; no legal/regulator certainty; existing Foundation API responses; forbidden-copy + no-leak tests). Alternative next slice: ADR-0078 Stage 1 (Layer 1 schema + Layer 3 helper + Layer 4 read service) if Founder wants transcript substrate before UI consumption. Other forward-substrate (each requires separate Founder authorization): ADR-0078 Stage 3 (governed listener); Wave 9 Option C BEAM-orchestrated; Wave 10 hierarchy; Wave 10 organizational graph.
