...
- **Foundation main HEAD:** `e1ac631` (VF.3 LocalMockVoiceProvider + integration tests PR #212 LIVE; **VF.4a Foundation voice intent route in flight**). **CT main HEAD:** `4e19e07` (PR #27 — CT C4-A Jira Cloud admin path LIVE). Sections 5 + 7 + 9 Approvals + 9 Policies remain PRODUCTION-GRADE COMPLETE. **VF.4a LANDING per ADR-0085 §8 (autonomous-continuation-authorized)** — Classification C backend runtime. NEW `apps/api/src/routes/voice.routes.ts` (POST /api/v1/voice/intents consuming constructEnvelope from VF.2; bearer auth via requireAuth("read"); caller_entity_id from request.auth; tenant_org_entity_id via resolveOrgOrFail; body validation against 13-surface enum + LOW/MEDIUM/HIGH intent_class + optional STANDARD/AGGREGATE_ONLY/EPHEMERAL retention + optional NON_WORK/PROTECTED_ATTRIBUTE/FORBIDDEN_INTENT redaction; 11-code closed-vocab failure enum). MOD `server.ts` registers registerVoiceRoutes. MOD barrel exports registerVoiceRoutes + VoiceIntentFailureCode. SAFE response shape: {intent_id, audit_event_id, source_surface, intent_class, confirmation_state, approval_chain_state, transcript_redacted, retention_class, created_at} — NEVER transcript_text / caller_entity_id / tenant_org_entity_id. NEW `tests/integration/voice-routes.test.ts` (11 tests across happy path LOW/MEDIUM/HIGH + audit row VOICE_INTENT_RECEIVED actor/target/SAFE schema verification + transcript-prose-NEVER-in-response invariant + 4 validation paths + 2 auth paths). Typecheck 4-error baseline preserved. NO Sesame; NO audio; NO CT change. **Next recommended:** **VF.4b** (CT voice surface scaffolding — text-only talk button on AI Twin page consuming POST /api/v1/voice/intents via api.voice.* namespace; confirmation modal for MEDIUM-tier; CT tests assert privacy invariant — requires repo-switch to otzar-control-tower; autonomous-continuation-authorized per ADR-0085 §8) OR **CT C4-B Linear admin path** OR **C-GitHub runtime**. VF.5+ require explicit Founder authorization per Sesame readiness assessment closeout.
- **Latest merged PR (Foundation):** [#212](https://github.com/NiovArchitect/niov-foundation/pull/212) — VF.3 LocalMockVoiceProvider + integration tests (envelope-to-audit-chain end-to-end). **Latest CT PR:** [#27](https://github.com/NiovArchitect/otzar-control-tower/pull/27) — CT C4-A Jira Cloud admin path.
...
- **Active branch / PR (Foundation):** `foundation-voice-first-vf-4a-route` — Classification C VF.4a Foundation route POST /api/v1/voice/intents per ADR-0085 §8 (autonomous-continuation-authorized). NEW voice.routes.ts + server.ts registration + 11 NEW integration tests + barrel + docs refresh.
