...
- **Section 10 production-readiness audit LANDED 2026-06-01** per `[FOUNDER-SECTION-10-PRODUCTION-READINESS-AUDIT-AUTH]` + 3 Founder addendums (OOTB role/tool/workflow templates + delegated authority/permission bundles/onboarding question sets + Dandelion activation layer). Read-only audit at `docs/current-build-state/10-deployment-security-go-live-operations.md`. **3 hard launch blockers**: (1) Section 8 Billing stub-only; (2) Section 4 first real adapter (decision-blocked); (3) **OOTB templates + Dandelion activation layer MISSING** — Foundation's existing `dandelion.service.ts` handles only org-admin invite Phase 2/3/4, NOT the company/department/user role-discovery / Twin-starter-profile / connector-priority-calculation flow the Founder describes. **Recommended next slice**: **ADR-0080 design-only** (OOTB + Dandelion ontology covering RoleTemplate / DepartmentTemplate / CompanyTemplate / ToolProfile / WorkflowTemplate / ConnectorPreset / DelegatedAuthorityProfile / PermissionBundle / OnboardingQuestionSet / AhaMomentPack / SafeFallbackMode / OrgChartRelationshipTemplate / IndustryVariant / CompanySizeVariant / DandelionFlowTemplate / DigitalTwinStarterProfile). Then seed catalog (worked example: EA Twin with SAP Concur + delegated authority). Then CT Dandelion preview (company + department + user setup). Then Dandelion-driven connector-priority matrix → first real Section 4 connector. Critical governance posture: Dandelion suggests; Foundation governance authorizes.
