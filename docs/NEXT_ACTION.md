...
- **Foundation main HEAD:** `5723f3b` (ADR-0084 PR #178 LIVE; **Section 8 B2 static entitlement catalog in flight**). **CT main HEAD:** `bf7f826`. Sections 5 + 7 + 9 Approvals + 9 Policies remain PRODUCTION-GRADE COMPLETE. **5-ADR Domain General Otzar design chain COMPLETE**: Wave 2.1 + ADR-0081 + ADR-0082 (base+Amendment 1) + ADR-0083 (base+Amendment 1) + ADR-0084 all LIVE. **Section 8 B2 static entitlement catalog LANDED** — `docs/entitlement-catalog/` 10 files / 93 items + `scripts/validate-entitlement-catalog.mjs` validator green. Implementation roadmap (Dandelion D2-D8 + Billing B3-B8 + Connectors C2-C9+ + Workflows W3-W6) bounded. Recommended next slice: B3 CT billing preview (read-only) OR D2 Dandelion Assessment substrate OR static connector implementation-readiness catalog (Slack/Google/Jira before C2 runtime). C2 real Slack connector remains Founder-gated.
- **Latest merged PR (Foundation):** [#178](https://github.com/NiovArchitect/niov-foundation/pull/178) — ADR-0084 Section 4 MCP/Connector Strategy. **Latest CT PR:** [#19](https://github.com/NiovArchitect/otzar-control-tower/pull/19) — Wave 6 surface LIVE.
