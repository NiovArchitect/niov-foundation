...
- **Foundation main HEAD:** `5b0b607` (Section 10 production-readiness audit PR #164 LANDED 2026-06-01; ADR-0080 OOTB + Dandelion ontology design-only in flight as next docs-only PR). **CT main HEAD:** `f4c24dd`. Sections 5 + 7 + 9 Approvals + 9 Policies remain PRODUCTION-GRADE COMPLETE for current scope. **ADR-0080 closes one of three hard launch blockers at the architecture register** (OOTB role/tool/workflow templates + Dandelion activation layer ontology). Remaining hard blockers: Section 8 Billing stub-only + Section 4 first-adapter decision. **No code / no schema / no routes / no seed / no runtime behavior** in ADR-0080 — pure design ADR.
- **Latest merged PR (Foundation):** [#164](https://github.com/NiovArchitect/niov-foundation/pull/164) — Section 10 production-readiness audit READ-ONLY + OOTB + delegated-authority + Dandelion addendums.
...
- **ADR-0080 LANDED design-only 2026-06-01** per `[FOUNDER-ADR-0080-OOTB-DANDELION-ONTOLOGY-DESIGN-ONLY-AUTH]`. `docs/architecture/decisions/0080-out-of-the-box-role-tool-workflow-connector-dandelion-ontology.md` (NEW). 16 design objects (RoleTemplate / DepartmentTemplate / CompanyTemplate / ToolProfile / WorkflowTemplate / ConnectorPreset / DelegatedAuthorityProfile / PermissionBundle / OnboardingQuestionSet / AhaMomentPack / SafeFallbackMode / OrgChartRelationshipTemplate / IndustryVariant / CompanySizeVariant / DandelionFlowTemplate / DigitalTwinStarterProfile) + role taxonomy + tool taxonomy + EA deepest worked example + three-tier Dandelion flow + connector prioritization model + RULE 0 governance posture + 8-wave implementation ladder. **NO code / no schema / no routes / no seed / no runtime behavior / no new audit literal / no Control Tower UI / no mutation to existing `dandelion.service.ts`**. Section 10 audit (PR #164) flagged 3 hard launch blockers — ADR-0080 closes the OOTB-template-missing blocker at the architecture register. Remaining hard blockers: Section 8 Billing stub-only + Section 4 first-adapter decision. **Recommended next slice (subject to Founder authorization)**: Wave 2 static seed catalog with Executive Assistant deepest worked example. **Do NOT implement Wave 2 automatically** — await separate Founder authorization. Alternative: Section 10 operational hardening pass (rollback runbook + admin bootstrap runbook + smoke-test suite) can land in parallel with Wave 2 preparation.
