...
- **Foundation main HEAD:** `70b1654` (CT D6 business admin walk closeout PR #201 LIVE; **D6 enterprise runtime in flight**). **CT main HEAD:** `612d825` (PR #25 — D6 business admin walk LIVE). Sections 5 + 7 + 9 Approvals + 9 Policies remain PRODUCTION-GRADE COMPLETE. **D6 enterprise archetype LANDED** — NEW `executeEnterpriseActivationForCaller` + NEW route `POST /api/v1/org/dandelion/activate/enterprise` + 10 NEW integration tests. 14-step enterprise plan composes against C2 OPERATING SLACK_READ + C3 RUNTIME_READY GOOGLE_WORKSPACE_READ (steps 8 + 9). Steps 5 (delegated authority) + 6 (break-glass registry) + 7 (LawfulBasis attestation) + 12 (board observer) emit audit-only at this slice. DUAL-CONTROL audit literals at steps 10 + 11 truthfully record design-intent; actual DUAL-CONTROL approval flow per ADR-0026 forward-substrate. Partial-failure: Slack binding stays LIVE if Google fails. Integration suite 242 → 252; typecheck 4-error baseline preserved; no-leak guard rebased 1181 → 1182. **Stage F graduation:** OPERATING (starter-pilot + team + business) → **OPERATING (all 4 archetypes) — the D6 4-archetype series at runtime is COMPLETE**. All 4 Dandelion plan archetypes from ADR-0080 Amendment 1 Stage F are now LIVE at the Foundation backend tier. **Next recommended:** **CT /onboarding D6 enterprise archetype admin walk** (Classification D; extend Onboarding.tsx with EnterpriseActivationCard collecting same Slack + Google fields → POST enterprise route → 14-step audit lineage with DUAL-CONTROL badges + audit-only step disclosures) OR **W4 Proposed Action substrate** (Classification A; Stage 3 promotion catalog consuming W3 entries) OR **C-Jira/Linear read-first connector runtime** (Classification E; mirrors C2/C3) OR **D6 DUAL-CONTROL middleware wiring** (Classification E; wires requireDualControl preHandler in front of /activate/enterprise route per ADR-0026).
- **Latest merged PR (Foundation):** [#200](https://github.com/NiovArchitect/niov-foundation/pull/200) — D6 business-archetype activation runtime. **Latest CT PR:** [#25](https://github.com/NiovArchitect/otzar-control-tower/pull/25) — D6 business-archetype activation admin walk.
