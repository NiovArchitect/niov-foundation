...
- **Foundation main HEAD:** `0315409` (D6 impl PR #196 LIVE; **D6 team archetype runtime in flight**). **CT main HEAD:** `5b1acc9` (PR #23 — D6 starter-pilot admin walk LIVE). Sections 5 + 7 + 9 Approvals + 9 Policies remain PRODUCTION-GRADE COMPLETE. **CT D6 admin walk LANDED** — NEW `src/lib/dandelion-activation/{types,labels}.ts` (discriminated result types + customer-admin vocabulary translation of 6 catalog audit_literals with safe fallback) + MOD api.ts (NEW dandelionActivation namespace) + MOD Onboarding.tsx (DandelionActivationCard; click → POST → 6-step timeline) + 26 NEW tests (CT suite 284 → 310). Privacy invariant: no raw ADMIN_ACTION: prefix leakage; 15-phrase forbidden UI copy guard. Closed-vocab failure-code → customer-admin message mapping for all 7 failure codes. D6 starter-pilot is now **OPERATING at Foundation backend tier + admin-visible at CT tier**. **Next recommended:** **D6 impl team archetype** (Classification E; consumes team-activation.json catalog; 8 steps including Slack connector binding registration via C2 SLACK_READ provider + /api/v1/org/connectors registration flow) OR **W4 Proposed Action substrate** (Classification A; Stage 3 promotion catalog) OR **C-Jira/Linear read-first connector runtime** (Classification E; mirrors C2/C3 pattern; ADR-0084 matrix C4-A/C4-B candidates).
