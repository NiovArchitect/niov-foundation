...
- **Foundation main HEAD:** `f8ea7aa` (D6 enterprise runtime PR #202 LIVE; **D6 DUAL-CONTROL middleware wiring in flight; closeout #203 awaiting merge**). **CT main HEAD:** `42e6b0c` (PR #26 — D6 enterprise admin walk LIVE; D6 4-archetype CT visibility series COMPLETE). Sections 5 + 7 + 9 Approvals + 9 Policies remain PRODUCTION-GRADE COMPLETE. **D6 DUAL-CONTROL middleware wiring LANDED** — MOD privileged-endpoints.ts (EscalationActionDescriptor +ORG_DANDELION_ENTERPRISE_ACTIVATION; NEW registry entry; size 5 → 6; Class B size 1 → 2) + MOD org.routes.ts (NEW imports + registry lookup with throw-guard + ordered preHandler [requireAdminCapability, requireDualControl]) + MOD privileged-endpoints test (+2 NEW tests) + MOD integration HTTP-tier (4 NEW dual-control tests: 503 fail-closed for single-admin org + 403 + PENDING for second-admin org + 200 + 14 steps for APPROVED grant + non-enterprise routes single-actor preserved) + MOD no-leak guard (line rebased 1182 → 1200). Integration 252 → 254. **Stage F enterprise route graduation:** OPERATING (single-actor) → **OPERATING (DUAL-CONTROL-enforced per ADR-0026)** — the CT DUAL-CONTROL badge from CT PR #26 now corresponds to actual runtime enforcement. Starter-pilot / team / business routes intentionally remain single-actor. **Next recommended:** **Section 10 operational hardening pass** (Classification A; audit recently-merged D6 + dual-control substrate for production-readiness gaps; update Section 10 audit doc) OR **C-Jira/Linear read-first connector runtime** (Classification E; mirrors C2/C3 pattern; ADR-0084 matrix C4-A/C4-B) OR **W4 Proposed Action substrate** (Classification A; Stage 3 promotion catalog) OR **D6 implementation extensions** (Classification E; forward-substrate gaps — BreakGlassGrant registry + LawfulBasisAttestation surface + BoardObserverScope tables for steps 6 / 7 / 12 audit-only at this slice).
