...
- **Foundation main HEAD:** `f8ea7aa` (D6 enterprise runtime PR #202 LIVE; **D6 DUAL-CONTROL middleware wiring in flight**). **CT main HEAD:** `42e6b0c` (PR #26 — D6 enterprise admin walk LIVE; D6 4-archetype CT visibility series COMPLETE). Sections 5 + 7 + 9 Approvals + 9 Policies remain PRODUCTION-GRADE COMPLETE. **CT D6 enterprise admin walk LANDED** — NEW EnterpriseActivationCard with 6 form fields in 2 sections + ActivationStepCard extended to detect `*_DUAL_CONTROL` audit_literal endings and render NEW DUAL-CONTROL badge (truthfully recording catalog design-intent) + 6 NEW labels covering steps 3 / 6 / 7 / 10 / 11 / 12 with audit-only forward-substrate disclosures and DUAL-CONTROL design-intent + 34 NEW tests (CT suite 377 → 411). Privacy invariant 4-layer: env-var NAME placeholders; success panel never contains xoxb-* / ya29.* / private-key JSON / Bearer even when admin pastes concrete tokens by mistake; no raw ADMIN_ACTION: prefix; 14-phrase forbidden UI copy guard. **D6 4-archetype CT visibility series matches backend completion** — all 4 Dandelion plan archetypes (starter-pilot + team + business + enterprise) are admin-visible at CT for both backend + UI tiers. **Next recommended:** **D6 DUAL-CONTROL middleware wiring** (Classification E; wires `requireDualControl(EnterpriseActivationDescriptor)` preHandler in front of `/activate/enterprise` route per ADR-0026; converts truthfully-recorded DUAL-CONTROL design-intent into actual approval-flow enforcement at runtime; closes meaningful operating-state gap with bounded scope) OR **Section 10 operational hardening pass** (Classification A; audit recently-merged D6 substrate for production-readiness gaps) OR **C-Jira/Linear read-first connector runtime** (Classification E; mirrors C2/C3 pattern; ADR-0084 matrix C4-A/C4-B) OR **W4 Proposed Action substrate** (Classification A; Stage 3 promotion catalog).
- **Latest merged PR (Foundation):** [#202](https://github.com/NiovArchitect/niov-foundation/pull/202) — D6 enterprise-archetype activation runtime. **Latest CT PR:** [#26](https://github.com/NiovArchitect/otzar-control-tower/pull/26) — D6 enterprise-archetype activation admin walk.
