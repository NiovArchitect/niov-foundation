...
- **Foundation main HEAD:** `def59e0` (VF.4a Foundation voice intent route PR #213 LIVE; **Foundation closeout for CT PR #28 VF.4b in flight**). **CT main HEAD:** `a1a99c7` (CT PR #28 — VF.4b voice talk surface LIVE). Sections 5 + 7 + 9 Approvals + 9 Policies remain PRODUCTION-GRADE COMPLETE. **Voice-first product arc closes at OPERATOR-VISIBLE register** — CT `/voice` page consumes `POST /api/v1/voice/intents` end-to-end against the LIVE Foundation route. Admins can compose voice intents through a real HTTP round-trip; risk-tier confirmation modal fires for MEDIUM/HIGH; LOW intents skip the modal per ADR-0085 §3. Text-only by design — NO microphone access, NO audio capture, NO browser MediaRecorder API; Sesame runtime + audio capture forward-substrate at VF.5+ (Founder-gated per Sesame readiness assessment 10 gates PENDING). CT artifacts: NEW `src/lib/voice/{types,labels}.ts` + NEW `src/pages/VoiceTwin.tsx` + MOD `src/lib/api.ts` `api.voice.*` namespace + NEW `/voice` route + Mic NAV entry + 26 NEW VoiceTwin tests + 442/442 full CT suite + typecheck/lint/build all green. Privacy invariant proven across 4 layers (SAFE response shape preserved; no Bearer / no `xoxb-*` / no `ya29.*` / no Atlassian PAT leak even when operator types simulated secrets; no raw failure-code leak in error UI). **Next recommended:** **CT C4-B Linear admin path** (mirror CT PR #27 Jira pattern) OR **C-GitHub read-first connector runtime** OR **D6-ENTERPRISE-AUDIT-ONLY-TABLES** (Section 10 hardening item 2) OR **W4 Proposed Action substrate**. VF.5+ require explicit Founder authorization per Sesame readiness assessment closeout.
- **Latest merged PR (Foundation):** [#213](https://github.com/NiovArchitect/niov-foundation/pull/213) — VF.4a Foundation voice intent route (POST /api/v1/voice/intents). **Latest CT PR:** [#28](https://github.com/NiovArchitect/otzar-control-tower/pull/28) — VF.4b CT voice talk surface (text-only; no microphone access).
...
- **Active branch / PR (Foundation):** `foundation-closeout-ct-voice-first-vf-4b` — Classification A docs-only closeout for CT PR #28. Records VF.4b OPERATOR-VISIBLE graduation across voice-first README + NEXT_ACTION; mirrors prior Foundation-closeout-after-CT cadence (PR #208 closeout for CT #27 etc).
