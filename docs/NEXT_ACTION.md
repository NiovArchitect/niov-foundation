...
- **Main HEAD:** `44cf12c` (ADR-0078 Conversation Substrate design-only LANDED 2026-05-31 PR #155). CT main HEAD: `ade4981` (Wave 10 ActionLifecyclePanel LIVE). Section 5 Agent Playground LIVE end-to-end + ADR-0078 four-layer substrate doctrine LIVE design-only across both repos.
- **Latest merged PR (Foundation):** [#155](https://github.com/NiovArchitect/niov-foundation/pull/155) — Add ADR-0078 design-only — Conversation Substrate (Source-of-Truth Transcripts + conversation_context_signals[] Safe-Projection Layer + Permissioned Drilldown + Capture-Eligibility/Work-Relevance Filtering) for Agent Playground.
- **Active branch / PR:** `adr-0079-transcript-substrate-policy` (ADR-0079 design-only Policy ADR — 33 sections turning ADR-0078 §6 + §6A + §6B + §6C into enforceable policy + 9 service-tier gates).
...
- **ADR-0079 LANDED 2026-05-31** at `[FOUNDER-ADR-0079-TRANSCRIPT-SUBSTRATE-POLICY-ADR-AUTH]` (this branch) — design-only Policy ADR turning ADR-0078 §6 + §6A + §6B + §6C into enforceable policy + 9 service-tier gates (canCaptureTranscript / classifyConversationRelevance / canRetainTranscript / canUseForAgentPlayground / canDrillDownTranscript / canQuoteTranscript / canExportTranscript / canDiscloseToRegulator / canDeleteOrPseudonymizeTranscript). ZERO new audit literal — existing ADMIN_ACTION + details.action discriminator reused. ADR-0078 §7 Stage 1+ implementation now POLICY-UNBLOCKED (still requires separate Founder authorization at slice). **Recommended next slice: ADR-0078 Stage 2 approved-source projection** (uses already-LIVE safe sources CORRECTION_SIGNAL / ACTION_HISTORY / HIVE_CONTEXT / MANUAL_USER_INPUT; brings conversation_context_signals[] to Agent Playground without raw transcript ingestion). Other forward-substrate (each requires separate Founder authorization): ADR-0078 Stage 1 (Layer 1 schema + Layer 3 helper + Layer 4 read service); ADR-0078 Stage 3 (governed listener); Wave 9 Option C BEAM-orchestrated; Wave 10 hierarchy; Wave 10 organizational graph.
