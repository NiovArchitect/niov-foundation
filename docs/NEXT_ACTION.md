...
- **Main HEAD:** `e708fa7` (Section 5 Wave 5 Option A deterministic / template-first TypeScript candidate-generation implementation LANDED 2026-05-31).
...
- **Section 5 Wave 6 outcome-comparison contract ADR** — per ADR-0065 §7 + ADR-0072 §15. Design-only ADR locking how the candidate set produced by Wave 5 Option A composes into a closed-vocab comparison matrix (tradeoffs / risks / dependencies per candidate); NO employee scoring; NO probabilistic-claim fabrication; consumes Wave 5 Option A output as input.
