...
- **Foundation main HEAD:** `4a299be` (ADR-0080 design-only PR #165 LANDED 2026-06-01; **Wave 2 static seed catalog in flight as next PR**). **CT main HEAD:** `f4c24dd`. Sections 5 + 7 + 9 Approvals + 9 Policies remain PRODUCTION-GRADE COMPLETE for current scope. **ADR-0080 + Wave 2 catalog substrate close one of three hard launch blockers at the architecture register** (OOTB role/tool/workflow templates + Dandelion activation layer ontology + 187-item seed catalog under governed envelope doctrine). Remaining hard blockers: Section 8 Billing stub-only + Section 4 first-adapter decision. **Per Founder addendum: JSON is not the moat — governed context envelope is.**
- **Latest merged PR (Foundation):** [#165](https://github.com/NiovArchitect/niov-foundation/pull/165) — ADR-0080 design-only.
...
- **ADR-0080 LANDED design-only 2026-06-01** per `[FOUNDER-ADR-0080-OOTB-DANDELION-ONTOLOGY-DESIGN-ONLY-AUTH]` (PR #165) + **Wave 2 static seed catalog LANDED 2026-06-01** per `[FOUNDER-ADR-0080-WAVE-2-STATIC-SEED-CATALOG-AUTH]` + `[FOUNDER-ADR-0080-WAVE-2-ADDENDUM-GOVERNED-CONTEXT-TRANSACTION-ENVELOPE]` + ADR-0080 Amendment 1 (governed context envelope doctrine). `docs/ootb-catalog/` (NEW): 6 catalog JSON files (15 RoleTemplate w/ EA deepest, 10 DepartmentTemplate, 15 CompanyTemplate variants, ~95 ToolProfile, 30 WorkflowTemplate, 14 ConnectorPreset, 1 DandelionFlowTemplate) + JSON Schema + `scripts/validate-ootb-catalog.mjs`. 187 items pass envelope + cross-reference + forbidden-phrase + EA-Concur + canonical-line checks. **Per Founder addendum: JSON is not the moat — governed context envelope is.** Each file carries `envelope_defaults` with 13 envelope metadata fields. NO Prisma schema, NO routes, NO services, NO Control Tower UI, NO connector code, NO billing, NO LLM/Python/BEAM, NO new audit literal, NO mutation to `dandelion.service.ts`. **Recommended next slice (subject to Founder authorization)**: Wave 3 CT/Dandelion read-only preview OR Wave 6 connector-priority matrix output OR Section 10 operational hardening pass OR Section 8 Billing ADR. **Do NOT implement Wave 3+ automatically** — await Founder authorization.
