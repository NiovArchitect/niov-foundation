...
- **Foundation main HEAD:** `9eb968a` (C4-B Linear runtime PR #209 LIVE; **VOICE-FIRST readiness slice VF.1 in flight per [FOUNDER-CORRECTION — OTZAR IS VOICE-FIRST / SESAME IS CORE PRODUCT REQUIREMENT]**). **CT main HEAD:** `4e19e07` (PR #27 — CT C4-A Jira Cloud admin path LIVE). Sections 5 + 7 + 9 Approvals + 9 Policies remain PRODUCTION-GRADE COMPLETE. **VF.1 LANDING** — Classification A docs-only research+design. NEW ADR-0085 (Otzar Voice-First Product Doctrine; 9 sub-decisions; canonicalizes the 4 doctrine lines + voice obeys Foundation governance + risk-tiered LOW/MEDIUM/HIGH action model + `VoiceProviderAdapter` seam with 4 adapter slots TextOnly/LocalMock/Sesame/Future + `VoiceIntentEnvelope` substrate object + Sesame readiness 10 gates + 13-surface interaction map + VF.1→VF.7 implementation sequence) + NEW `docs/voice-first/` directory (9 files: README + doctrine + interaction-map + sesame-readiness-assessment + voice-provider-adapter + voice-intent-envelope + risk-tiered-action-model + implementation-sequence + JSON schema) + NEW `scripts/validate-voice-first.mjs` (pure Node ESM mirror of validate-dandelion-activation; 58 checks green) + MOD `docs/CURRENT_BUILD_STATE.md` global product directive line cites ADR-0085 canonicalization. NO Sesame API call; NO secrets; NO audio processing; NO runtime voice code; NO new audit literal in AUDIT_EVENT_TYPE_VALUES (forward-substrate at VF.2 per ADR-0042 §Q-γ.1). Section 4 Linear remains RUNTIME_READY; connector substrate posture unchanged. **Next recommended:** **VF.2** (TypeScript `VoiceProviderAdapter` + `TextOnlyVoiceProvider` + `VoiceIntentEnvelope` + closed-vocab audit literals — Classification C autonomous-continuation-authorized per ADR-0085 §8) OR **CT C4-B Linear admin path** (mirror CT PR #27) OR **C-GitHub read-first connector runtime** OR **W4 Proposed Action substrate**.
- **Latest merged PR (Foundation):** [#209](https://github.com/NiovArchitect/niov-foundation/pull/209) — C4-B Linear read-first connector runtime (Project/Engineering family closes at 2/2). **Latest CT PR:** [#27](https://github.com/NiovArchitect/otzar-control-tower/pull/27) — CT C4-A Jira Cloud admin path.
...
- **Active branch / PR (Foundation):** `foundation-voice-first-readiness` — Classification A docs-only voice-first readiness slice VF.1 per `[FOUNDER-CORRECTION — OTZAR IS VOICE-FIRST / SESAME IS CORE PRODUCT REQUIREMENT]`. NEW ADR-0085 + NEW `docs/voice-first/` (9 files) + NEW validator + MOD `docs/CURRENT_BUILD_STATE.md` global product directive citing ADR-0085 canonicalization.
