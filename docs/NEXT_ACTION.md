...
- **Foundation main HEAD:** `dcffc3c` (VF.1 voice-first doctrine PR #210 LIVE; **VF.2 voice runtime substrate in flight**). **CT main HEAD:** `4e19e07` (PR #27 — CT C4-A Jira Cloud admin path LIVE). Sections 5 + 7 + 9 Approvals + 9 Policies remain PRODUCTION-GRADE COMPLETE. **VF.2 LANDING per ADR-0085 §8 (autonomous-continuation-authorized)** — Classification C backend runtime. NEW `apps/api/src/services/voice/` directory: `voice-provider.service.ts` (VoiceProviderAdapter interface + 4-slot dispatch + 8-code closed-vocab failure enum mirroring ConnectorResult); `text-only-voice.provider.ts` (TextOnlyVoiceProvider — typed input emits envelope; always available; no secrets; no env-var gate); `voice-intent-envelope.ts` (VoiceIntentEnvelope substrate object + constructEnvelope service emitting VOICE_INTENT_RECEIVED audit BEFORE delivery per RULE 4 + emitVoiceLifecycleAudit for 5 lifecycle literals + 13-surface VOICE_SOURCE_SURFACES frozen anchor + LOW→NOT_NEEDED+NONE / MEDIUM→PENDING+NONE / HIGH→PENDING+PENDING risk-tier discrimination). 6 NEW closed-vocab audit literals APPENDED to AUDIT_EVENT_TYPE_VALUES (VOICE_INTENT_RECEIVED/CONFIRMED/REJECTED/EXPIRED/REDACTED/DELIVERED) per ADR-0042 §Q-γ.1 clean-transition. Privacy invariant: audit details schema SAFE (intent_id + source_surface + intent_class + states + transcript_redacted + reason + retention_class) — NEVER transcript_text / Bearer / OAuth / secret / proposed_action body. 44 NEW unit tests + frozen-anchor contracts (4 provider slots; 13 source surfaces); unit suite 1282 → 1326. Typecheck 4-error baseline preserved. NO Sesame; NO audio; NO new route; NO CT change. **Next recommended:** **VF.3** (LocalMockVoiceProvider + fixture catalog + integration tests asserting envelope-to-Action-runtime end-to-end — autonomous-continuation-authorized) OR **VF.4** (CT voice surface scaffolding — text-only talk button) OR **CT C4-B Linear admin path** OR **C-GitHub runtime**.
- **Latest merged PR (Foundation):** [#210](https://github.com/NiovArchitect/niov-foundation/pull/210) — VF.1 Otzar Voice-First Product Doctrine (ADR-0085 + docs/voice-first/ substrate + validator). **Latest CT PR:** [#27](https://github.com/NiovArchitect/otzar-control-tower/pull/27) — CT C4-A Jira Cloud admin path.
...
- **Active branch / PR (Foundation):** `foundation-voice-first-vf-2-runtime` — Classification C VF.2 voice runtime substrate per ADR-0085 §8 (autonomous-continuation-authorized). NEW `apps/api/src/services/voice/` (3 files) + 6 NEW audit literals appended to AUDIT_EVENT_TYPE_VALUES + 44 NEW unit tests + barrel exports via @niov/api + docs refresh.
