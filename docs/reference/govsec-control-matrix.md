...
| GAP-B2 | B API | gateway static limits | per-op limits + **G4-B2-A adversarial-sim harness landed** (single-source floods shed by G4-A; distributed-under-limit swarm = residual, NOT shed). Production swarm counter (hashed-IP cluster + setMultiplier) = **G4-B2-B after G4-D** (coupled to GAP-O2 hot-key). **Behavioral closure NOT done.** **G4-D-D3: G4-D complete (D1 baseline + D2-A `hit` Lua optimization verified per GAP-O2); G4-B2-B unblocked.** **G4-B2-B (this commit): GAP-B2 behaviorally CLOSED — production swarm counter LANDED (Fork α direct cluster shed). Aggregate cluster counter `swarm:<op>:cluster:<bucket>` (`bucket = HMAC-SHA256(ip, jwtSecret) % 64`, no raw IP/PII) via the existing `store.hit` fixed-window; per-key limit stays primary; cluster breach → 429. The two G4-B2-A residual distributed-under-limit swarm cases now SHED. No `setMultiplier`/second `getMultiplier` (Fork α; D2-B deferred); no `SWARM_DETECTED` literal; no ADR-0002 amendment; logger-only swarm denials. Governed op-count now 2 hit + 1 getMultiplier (the budgeted cost gated on G4-D). GAP-O7 remains open; no CI p99/timing assertions.** | High | High | GOVSEC.4 | code | adversarial sim (harness landed G4-B2-A) | RATE_LIMITED/logger (G4-B1); no new literal | core: swarm backpressure (G4-B2-B) | swarm load shed without correctness loss (G4-B2-B, post-G4-D) | B |
...
| GAP-B4 | B API/C | privileged routes | dual-control | privileged endpoints unthrottled | Med | Med | GOVSEC.4 | code | integration | rate-limit audit | — | **G4-C (this commit): throttle slice CLOSED for the 4 dual-control `PRIVILEGED_ENDPOINTS` routes** (PATCH /platform/monetization/config, POST /platform/orgs, POST /regulator/access-grants, POST /regulator/access-revocations). Framing correction: these were never literally unthrottled post-G4-A — G4-A governed them at the generous `default` fallback (300/min); G4-C maps them to a strict `privileged` gateway op (5/min, entity-scoped, matching the admin_reset posture). Gateway data-table only (OPERATION_RULES + DEFAULT_LIMITS); `detectOperation` logic, dual-control middleware, `requireDualControl`, auth-admin routes, route preHandlers all unchanged; reuses the G4-B1 `RATE_LIMITED` audit (no new literal, no ADR-0002); op-count budget unchanged. **The broader org-admin `requireAdminCapability` route set remains a GOVSEC.5 follow-on.** **Org-admin route-set throttle (this commit): the org-admin follow-on is CLOSED.** A NEW gateway `admin` operation class (60/min, entity-scoped; `DEFAULT_LIMITS.admin` + `SWARM_DEFAULT_LIMITS.admin = 1200` per the ~perMinute×20 convention) classifies the broader `requireAdminCapability` surface that G4-C left on the generous `default` (300/min): `/api/v1/org/*` (GET/POST/PATCH/DELETE), the non-privileged `/api/v1/platform/*` reads, `POST /api/v1/auth/admin-register`, `POST /api/v1/otzar/domain/vocabulary`, and the `/api/v1/break-glass/*` routes. Gateway data-table only (`DEFAULT_LIMITS` + `SWARM_DEFAULT_LIMITS` + `OPERATION_RULES`, appended AFTER the privileged/admin_reset rules so first-match ordering preserves them); `detectOperation`, `rate-limit.ts`, `RedisRateLimitStore`, `HIT_LUA`, `RateLimitStore`, `getMultiplier`/`setMultiplier`, `admin.middleware.ts`, route handlers, `privileged-endpoints.ts` all unchanged. The 4 dual-control `PRIVILEGED_ENDPOINTS` routes stay `privileged` (5/min); `POST /auth/admin-reset` stays `admin_reset` (5/min); regulator grant/revoke stay `privileged`; `/otzar/observe` + `/otzar/correction` are NOT admin-throttled. Reuses the `RATE_LIMITED` audit (no new literal, no ADR-0002). G4-C throttle + G4-B2-B swarm unchanged; GAP-C1 preserved; GAP-K1 remains CLOSED; ADR-0050 remains Accepted; BG.2 behavior unchanged. **GOVSEC.5 remains OPEN** — the only remaining GOVSEC.5 item is **GAP-K2** least-privilege capability review (open / undocumented; a throttle is not a least-privilege review). GAP-O7 remains open; D2-C / GOVSEC.7 deferred. **G4-C is a GOVSEC.4-tail throttle slice coordinated with GOVSEC.5; GOVSEC.5 is NOT closed** (dual-control self-approval resolution + break-glass/time-boxed audit remain open). GAP-O7 remains open; no CI p99/timing assertions. | H |
| GAP-C1 | C Admin | `dual-control.middleware.ts` | placeholder target=caller | self-approval possible | High | Low | GOVSEC.5 | code | unit+integration | ADMIN_ACTION audit | — | **G4-C → self-approval resolution (this commit): RESOLVED.** `escalation.service.ts` `transitionPendingForCaller` now enforces the two-person invariant — the initiator (`source_entity_id`) may never approve/resolve their own escalation (`caller === source → ESCALATION_FORBIDDEN`), enforced BEFORE the target/resolver gate so it holds even for the self-target dual-control placeholder (`target_entity_id === source_entity_id`) that previously let `caller === target` self-resolve. A distinct second human (target / designated resolver) is still required; distinct-approver success path preserved. `requireDualControl` read-side unchanged; reuses existing `ESCALATION_FORBIDDEN` + `ADMIN_ACTION` audit (no new literal, no ADR-0002). **Substrate note:** the distinct-approver-target *creation* path (org-admin-set resolution) remains forward-queued, so the self-target dual-control escalation is now fail-closed (un-approvable) — safe; the 4 G4-C privileged routes require a distinct approver. **GOVSEC.5 still OPEN** (break-glass / time-boxed audit GAP-K1 remains). | B |
...
| GAP-K1 | K Insider | dual-control | two-person | no break-glass | Med | Low | GOVSEC.5 | code+policy | integration | break-glass audit | resilience when approver locked out | break-glass with time-boxed two-person audit — **ADR-0050 (this commit): DESIGNED, not closed.** Design-first ADR (Proposed) locks a time-boxed emergency grant (mandatory `valid_until`; explicit justification; scoped to the 4 dual-control PRIVILEGED_ENDPOINTS initially; normal dual-control remains default; GAP-C1 self-approval guard intact; future `BreakGlassGrant`-style table mirroring LawfulBasis; future append-only audit literals in audit.ts with no ADR-0002 amendment; mandatory post-hoc two-person audit/review). **No code/schema/audit/test in the ADR phase; code requires a separate EXECUTE-VERIFY-AUTH.** GOVSEC.5 remains OPEN until break-glass code + audit-completeness tests land. **BG.1 substrate-first (this commit): the break-glass SUBSTRATE landed — NEW Prisma `BreakGlassGrant` table (mandatory `valid_until`, `action_type` scope, review fields) + `BreakGlassStatus` enum (db-push, Prisma-only, no migration, no Ecto mirror) + 4 additive `BREAK_GLASS_*` audit literals (no ADR-0002 amendment) + NEW `break-glass.service.ts` (create/validate/markUsed/expire/review; mandatory justification + future `valid_until`; 4-action scope from PRIVILEGED_ENDPOINTS; single-use; self-review forbidden; per-mutation audit in-tx) + service-level unit tests. NO middleware/route wiring — NO live bypass (deferred to BG.2). GAP-K1 still NOT closed; GOVSEC.5 still OPEN; BG.2 integration + BG.3 closure remain.** **BG.2 live integration (this commit): the break-glass grant is now usable in the live dual-control request path — NEW `break-glass.routes.ts` (`POST /break-glass/grants` invoke + `POST /break-glass/grants/:grant_id/review` review, both `can_admin_niov`; registered in `server.ts`) + a recognition seam in `dual-control.middleware.ts` that fires ONLY in the Denied/PermanentFailure branch (after no APPROVED escalation; before get-or-create-PENDING + 403): `validateBreakGlassGrant(callerEntityId, actionDescriptor.type)` then `markBreakGlassUsed`, delegating ONLY if the consume succeeds (atomic ACTIVE→USED = single-use gate + TOCTOU close); normal APPROVED dual-control always wins first; expired/used/mismatched grants do not authorize; no request-path `expireBreakGlassGrant`. Audit: `BREAK_GLASS_USED` in-tx + a `DUAL_CONTROL_BREAK_GLASS_DELEGATED` marker on the existing `ADMIN_ACTION` event_type (no new literal, no `audit.ts` change, no ADR-0002 amendment); no justification leak. NEW `tests/integration/break-glass-integration.test.ts`. No schema / `privileged-endpoints.ts` / platform-or-regulator-handler / `gateway.middleware.ts` / G4-C / `escalation.service.ts` change; GAP-C1 intact. GAP-K1 STILL NOT closed; GOVSEC.5 STILL OPEN — closure is BG.3 (ADR-0050 Proposed→Accepted + GAP-K1 closed).** **BG.3 closure (this commit): GAP-K1 CLOSED.** Break-glass / time-boxed two-person audit is implemented + accepted across ADR-0050 (**Accepted 2026-05-22**) + BG.1 substrate (`2f487fc`) + BG.2 live integration (`f5f0256`): mandatory `valid_until`, explicit justification, 4-action `PRIVILEGED_ENDPOINTS` scope, single-use ACTIVE→USED consumption, self-review rejection, audit completeness (`BREAK_GLASS_INVOKED`/`USED`/`EXPIRED`/`REVIEWED` + `DUAL_CONTROL_BREAK_GLASS_DELEGATED` marker), no justification leak, denied-branch seam, green CI (run `26297686163`). Docs-only closure cascade; no code/schema/audit/test change. **GOVSEC.5 remains OPEN** because (a) the broader org-admin `requireAdminCapability` route-set throttle remains a follow-on (not started) and (b) **GAP-K2** least-privilege capability review remains open / undocumented. RULE 13 drift recorded in ADR-0049: the narrow GOVSEC.5 closure checklist now reads as satisfied (self-approval + break-glass + privileged throttle), but the broader phase scope (insider-threat / org-admin / GAP-K2) holds GOVSEC.5 open per Founder disposition. GAP-O7 remains open; D2-C / GOVSEC.7 deferred. | H |
| GAP-K2 | K Insider | TAR caps | clearance ceiling | no least-privilege capability review | Low | Low | GOVSEC.5 | policy-runbook | — | review record | — | least-privilege review documented — **CLOSED / documented (this commit): `docs/reference/govsec-least-privilege-review.md`.** The policy-runbook review record inventories the TAR capability model (8 `can_*` flags + `clearance_ceiling`), the `requireAdminCapability` two-tier admin model (`can_admin_niov` / `can_admin_org`), the route-to-capability mapping, and assesses the 4 PRIVILEGED_ENDPOINTS + break-glass + org-admin surfaces + clearance_ceiling against least-privilege. Finding: no immediate least-privilege violation requiring code — the model is coarse but defensible under layered defense-in-depth (dual-control + GAP-C1 self-approval guard + break-glass time-box/single-use/two-person-review + `privileged` 5/min + `admin_reset` 5/min + `admin` 60/min throttle + default-deny capabilities + RULE 0 AI ceilings + ADR-0002 audit). A future finer-grained `can_admin_org` sub-capability split is recorded as **forward-substrate only** (not required for GAP-K2 closure). Docs-only; no code/schema/audit/test change; no new audit literal; no ADR-0002. **GOVSEC.5 remains OPEN pending a separate closure cascade** (GAP-K2 closure does not flip GOVSEC.5). GAP-O7 remains open; D2-C / GOVSEC.7 deferred. | F |
...
| GAP-O2 | gateway rate-limit perf (`detectOperation` + Redis round-trips) under adversarial load | bot/swarm bounding | limit accuracy under burst | Redis INCR hot-key contention | GOVSEC.4 | latency p99 + Redis ops under burst | governed routes hold p99 target under adversarial burst — **G4-D-D1 (this commit): op-count baseline + local p99 runbook landed** (`docs/reference/govsec-perf-budget.md`; CI gates store-call budget, p99 is local since CI has no Redis). Baseline: `hit`=INCR+first-EXPIRE+TTL (~2-3 round-trips, not pipelined) + `getMultiplier` GET + auth ip_whitelist DB read ≈ 3-4 round-trips/governed req. **G4-D-D2-A (this commit): `hit` optimized to a single atomic Lua `EVAL` — 1 round-trip (down from ~2-3) + closes a latent no-TTL orphan-key race; `count`/`ttl_seconds`/Retry-After unchanged. `getMultiplier` (D2-B) + ip_whitelist DB read (D2-C → GOVSEC.7) deferred. Post-D2-A ≈ 2 Redis round-trips/governed req + DB read.** **Optimization landing = D2-A (`b6fe3b0`); verification = D3. G4-D-D3 (this commit): optimization verified + EVAL semantics verified + op-count budget verified (CI run `26265354599` green across Typecheck/Unit/Integration/Elixir); GAP-O2 = optimization-verified under the documented local/manual p99 posture — Redis p99 / wall-clock burst behavior governed by the runbook (`docs/reference/govsec-perf-budget.md` §6/§9), NOT asserted as CI-closed; no CI p99/timing assertions added. G4-B2-B unblocked after D3 (post-optimization budget verified). **G4-B2-B (this commit): the swarm cluster counter landed — governed op-count is now 2 hit + 1 getMultiplier (the budgeted cost gated on G4-D); the 2nd `hit` is the D2-A atomic EVAL (one round-trip). GAP-O2 stays optimization-verified under the documented local/manual p99 posture; still not CI-p99-closed.** **G4-D-D2-B (this commit, docs-only no-op): `getMultiplier` analyzed — already one minimal Redis GET / O(1), one call site, no second call from B2-B; caching rejected (stale-multiplier risk), producer-scoped skipping rejected (silent-control-failure footgun), combined-EVAL rejected (forbidden interface/HIT_LUA change). No code/test change; budget unchanged. Next real perf lever = D2-C (ip_whitelist DB read), deferred to GOVSEC.7.** GAP-O7 (working-set route p99) remains open — NOT closed here.** |
...
| GAP-O7 | working-set route latency under adversarial request volume | LLM10 unbounded consumption | consumer-view correctness | concurrent working-set builds | GOVSEC.4 | route p99 under volume | route bounded + backpressured under adversarial volume — **G4-D-D1 covers the gateway op-budget (GAP-O2) only; working-set-route-specific p99 measurement is a focused follow-on sub-step, NOT closed by D1.** |
...
| GOVSEC.5 | self-approval resolved; break-glass with time-boxed audit; privileged routes throttled; tests pass — **PHASE CLOSED 2026-05-22.** All GOVSEC.5-scoped items complete + CI-green: GAP-C1 self-approval resolution (`98c7351`); GAP-K1 break-glass CLOSED (ADR-0050 Accepted + BG.1 `2f487fc`/CI `26295014869` + BG.2 `f5f0256`/CI `26297686163` + BG.3 `877b3c0`/CI `26298824244`); org-admin route-set throttle follow-on CLOSED (`f06be71`/CI `26301553439`, gateway `admin` op 60/min); GAP-K2 least-privilege review CLOSED/documented (`d491f21`/CI `26302426953`, `docs/reference/govsec-least-privilege-review.md`). **Drift reconciled by completion:** at BG.3 the narrow §closure checklist read satisfied while the broader phase scope (org-admin throttle + GAP-K2) was open; both are now satisfied, so GOVSEC.5 has no remaining open scoped gap. **ADR-0049 umbrella remains Proposed** — GOVSEC.5 is a closed phase inside the 10-phase umbrella, not full GOVSEC completion (GOVSEC.2/3-tail/4-tail/6–10 remain). Docs-only; no code/schema/audit/test change. **GAP-O7 remains open** (GOVSEC.4-tail optimization, not a GOVSEC.5 gap); D2-C / GOVSEC.7 deferred; GOVSEC.7 untouched. |
