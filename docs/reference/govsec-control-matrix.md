...
| GAP-O2 | gateway rate-limit perf (`detectOperation` + Redis round-trips) under adversarial load | bot/swarm bounding | limit accuracy under burst | Redis INCR hot-key contention | GOVSEC.4 | latency p99 + Redis ops under burst | governed routes hold p99 target under adversarial burst — **G4-D-D1 (this commit): op-count baseline + local p99 runbook landed** (`docs/reference/govsec-perf-budget.md`; CI gates store-call budget, p99 is local since CI has no Redis). Baseline: `hit`=INCR+first-EXPIRE+TTL (~2-3 round-trips, not pipelined) + `getMultiplier` GET + auth ip_whitelist DB read ≈ 3-4 round-trips/governed req. **G4-D-D2-A (this commit): `hit` optimized to a single atomic Lua `EVAL` — 1 round-trip (down from ~2-3) + closes a latent no-TTL orphan-key race; `count`/`ttl_seconds`/Retry-After unchanged. `getMultiplier` (D2-B) + ip_whitelist DB read (D2-C → GOVSEC.7) deferred. Post-D2-A ≈ 2 Redis round-trips/governed req + DB read.** **Optimization landing = D2-A (this commit); verification = D3; behavioral closure still pending D3. GAP-O7 (working-set route p99) NOT closed here.** |
