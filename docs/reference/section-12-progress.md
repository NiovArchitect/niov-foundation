...
| Phase 3 Sub-Arc 1 Sub-Phase d — DEVICE Cold-Shard Substrate per ADR-0040 | **IN FLIGHT** (`this commit`) | this commit | Phase 3 (Dynamic Memory Accuracy at Scale) sub-arc 1 sub-phase d Commit 1 of 4 per ADR-0040 §Sub-decision 7 4-commit mini-arc decomposition (D.1 ADR + D.2 module + D.3 dispatch + D.4 closure). ADR-0040 NEW (DEVICE Cold-Shard Substrate per ADR-0038 §Sub-decision 3 Forward Queue; Status Proposed 2026-05-17) embeds D.0 [BEAM-DBGI-DEVICE-COLDSHARD-RESEARCH-ARC] Rule 21 research arc canonical at canonical-knowledge register substantively (5 parallel WebSearch + 1 WebFetch on Discord ex_hash_ring; Jump Hash + Rendezvous + Ring/Ketama + Elixir Bitwise + library survey). ADR-0040 canonicalizes 11 sub-decisions at substrate-architectural register substantively: Jump Consistent Hash (Lamping-Veach 2014) + pure stateless module + CosmpRouter.DeviceShard at apps/cosmp_router/lib/cosmp_router/device_shard.ex (Founder Q-F OVERRIDE OF D.0 report; "DeviceShard" not "DeviceShardManager" because pure module not state-bearing) + K default 256 / range [128, 1024] + umbrella config/config.exs + explicit :device dispatch branch at D.3 + AI_AGENT remains canonical at PERSONAL branch + 4-commit decomposition + 6 BEAM-compatibility patterns preserved by construction + Elixir anti-pattern compliance + testability per ADR-0034 + patent-implementation evidence at canonical decision register substantively. Q-A through Q-F Founder Q-locks LOCKED at [BEAM-DBGI-DEVICE-COLDSHARD-QLOCK] register substantively at canonical decision register substantively. **Substrate-architectural framing canonical at substantive register**: Jump Consistent Hash mathematically optimal for fixed-K=128-1024 constraint canonical at canonical-architectural register substantively (O(1) space + O(ln K) compute + near-perfect balance + no virtual nodes + minimal-remap); pure stateless module avoids GenServer anti-pattern + supervised child + ETS hot path overhead at canonical-coherence register substantively; mirrors B.2 Operations pure-module refactor precedent canonical per ADR-0039 §Sub-decision 11 register substantively. **Substrate-state ground truth register**: sub-phase d targets DEVICE tier dispatch routing at canonical-execution register substantively per ADR-0038 §Forward Queue line 249 canonical at canonical-knowledge register substantively; **this commit substantively LOCKS the ADR-0040 architectural substrate for the K=128-1024 DEVICE cold-shard forward-substrate item; implementation closure remains forward-substrate to D.2 + D.3 and final closure to D.4 docs-only closure cascade register substantively.** **4-commit decomposition** per ADR-0040 §Sub-decision 7: Commit D.1 `[BEAM-DBGI-DEVICE-COLDSHARD-ADR]` docs-only at this commit; Commit D.2 `[BEAM-DBGI-DEVICE-SHARD-MODULE]` NEW DeviceShard pure module + Jump Hash implementation + import Bitwise + config + unit tests forward-substrate; Commit D.3 `[BEAM-DBGI-DEVICE-SHARD-DISPATCH-INTEGRATION]` MOD grpc/server.ex explicit :device branch + dispatch_device_shard/3 + integration tests forward-substrate; Commit D.4 `[BEAM-DBGI-DEVICE-COLDSHARD-CLOSURE]` docs-only closure cascade forward-substrate (closes ADR-0038 §Forward Queue item at canonical-state register substantively). |
