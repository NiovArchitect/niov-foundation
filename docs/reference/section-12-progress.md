...
| 12.5 CAR Sub-box 2 — Jurisdiction Tagging (1.6 + 2.4) | **IN FLIGHT** (sub-phase 1 of 6) | this commit | **Distinct from progress tracker "Sub-box 2 Phase 1/Phase 2" above** per D-CAR-SUB-BOX-NUMBERING-DRIFT canonical at ADR-0035 §9 35th. CAR Sub-box 2 substrate: `Entity.jurisdiction` + `MemoryCapsule.jurisdiction` + `AuditEvent.jurisdiction` + `OrgSettings.default_jurisdiction` schema fields + `assertJurisdictionalScope()` runtime check at permission/share/read flows. CAR Sub-box 3 (REGULATOR + Lawful-Basis per ADR-0036) proceeded with its own self-contained `LawfulBasis.jurisdiction_invoked` field per Sub-box 3 pre-flight Q1 LOCKED Option A and CLOSED at `f0cdbb9`; CAR Sub-box 2 (Entity/Capsule/Audit jurisdiction tagging) was preserved as QUEUED at Sub-box 3 closure. **6-sub-phase mini-arc per ADR-0037 §Implementation Detail** (sub-phase 1 [CAR-SUB-BOX-2-ADR] this commit; sub-phases 2-6 implement schema → service helpers + defaulting → COSMP enforcement → REGULATOR integration → closure cascade). ADR-0037 (`docs/architecture/decisions/0037-jurisdiction-tagging-architecture.md`) LANDED at sub-phase 1 with 9 sub-decisions: (1) single-String jurisdiction representation per Q-NEW-1 LOCKED Option α (matches LawfulBasis.jurisdiction_invoked precedent); (2) field placement at 4 schema models (Entity + MemoryCapsule + AuditEvent + OrgSettings); (3) AuditEvent.jurisdiction is row-metadata-only per Q-NEW-3 LOCKED Option β (NOT extending canonical_record/1; preserves Sub-box 3 sub-phase 4 14-field byte-equivalence + 12 fixture pairs + cosmp_router default tier 137/0 unchanged); (4) MemoryCapsule.jurisdiction immutable after creation per Q-NEW-4 LOCKED Option α (cross-region transfer is forward-queued explicit workflow); (5) service-tier defaulting cascade per Q-NEW-6 LOCKED Option α (capsule from owner Entity → OrgSettings; entity from OrgSettings; Prisma cannot do cross-row defaults); (6) `assertJurisdictionalScope` pure-function design per Q-NEW-2 LOCKED Option α at NEW `apps/api/src/services/cosmp/jurisdiction-enforcement.ts` (mirrors sub-phase 6 of Sub-box 3 enforceRegulatorCOSMPAccess pattern; 6 BEAM-compatibility patterns from ADR-0026 §5 preserved by construction); (7) COSMP enforcement at NEGOTIATE start-check + readContent TOCTOU re-check + SHARE start-check + REVOKE start-check + WRITE create-time defaulting/immutability; (8) REGULATOR integration per Q-NEW-5 LOCKED Option α (LawfulBasis.jurisdiction_invoked === MemoryCapsule.jurisdiction match enforcement at sub-phase 5; augments — does NOT replace — TAR.regulator_jurisdiction check from Sub-box 3); (9) CAR Sub-box 2 enables downstream Sub-boxes 4 + 5 + 8 + 9. **Patent relevance: NONE directly** per CAR §1.6 verbatim ("region tagging is conventional"); ADR-0037 has NO Patent-Implementation Evidence section per Q-NEW-8 LOCKED Option α; NO ADR-0020 cite; NO ADR-0019 cite; NO ADR-0033 cite (Q-NEW-3 Option β preserves canonical_record/1 unchanged); ADR-0035 §9 NO promotion per Q-NEW-9 LOCKED Option α (any substrate-build observations remain commit-body-only per sub-phase 6 + sub-phase 7 of Sub-box 3 LOCK precedent). RULE 14 back-cite to ADR-0036 §Substrate-Honest Distinctions landed at this commit (closes the previously preserved QUEUED reference). Forward-queued (per ADR-0037 §Forward Queue): physical data residency enforcement; legal transfer determination engine; real-time country/legal rules engine; cross-region capsule transfer workflow; multi-jurisdiction capsule support; canonical_record/1 jurisdiction binding (cryptographic) if future evidence justifies; Cross-Tenant Compliance Benchmarking patent-relevance analysis per CAR §1.6 forward path; AuditEvent.jurisdiction automatic operation-context propagation refinement. NOT claimed: legal compliance certification; physical data residency enforcement; multi-region storage placement; cross-border transfer legal determination; per-country regulatory automation; real-time legal rules engine; full DMW-to-DMW orchestration; BEAM/Broadway backpressure implementation; Federation Cloud monetization; external PKI / EU eIDAS / national registry integration; direct patent relevance. |
