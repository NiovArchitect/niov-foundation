...
| Phase 3 Sub-Arc 1 Sub-Phase a — DMW Worker per ADR-0038 | **IN FLIGHT** (Commit 1 of 3) | this commit | Phase 3 (Dynamic Memory Accuracy at Scale) sub-arc 1 sub-phase a Commit 1 of 3 per SYNTHESIS-SUB-PHASE-A-DECOMPOSITION (ADR + CODE + CLOSURE). ADR-0038 NEW (DMW Worker per-DMW Supervised Process; Status Proposed 2026-05-15) + ADR-0028 §Forward Queue NEW append-only LANDED sub-paragraph (sub-arc 1 sub-phase a closure update; per-capsule supervised Elixir process forward-queue item progresses at per-DMW granularity per ADR-0038) + ADR-0028 §Bidirectional citations (cited from) NEW entry (RULE 14 back-citation) + catalog refreshes across architecture/README + CLAUDE + this row NEW + CURRENT_BUILD_STATE NEW H2 section. ADR-0038 canonicalizes 8 sub-decisions at substrate-architectural register: module location (`apps/dbgi_supervisor/lib/dbgi_supervisor/dmw_worker.ex`) + identity addressing (entity_id via Registry + Phoenix.Tracker topic) + tier dispatch axis (WalletType 3-tier PERSONAL/ENTERPRISE/DEVICE) + lifecycle pattern (lazy-spawn on first COSMP op) + state (stateless + Phoenix.Tracker presence only) + DMWWorker vs cosmp_router relationship (separate-layer; cosmp_router stays as-is at sub-phase a) + 6 BEAM-compatibility patterns from ADR-0026 §5 preserved + testability per ADR-0034. Q-A through Q-G operator-tier locks all approved at recommended defaults α at canonical decision register. **Hybrid hot/cold framing canonical at substantive register**: ENTERPRISE wallets run always-hot per-DMW supervised process + PERSONAL/AI_AGENT wallets promote-on-activity from cold shard substrate to hot per-DMW substrate + DEVICE wallets run always-cold shard-mapped substrate. **Substrate-state ground truth register**: sub-phase a delivers the per-DMW supervised process substrate that the BEAM scaffolding (sub-phases 8-11; :pg + Registry + DynamicSupervisor + Cluster.Supervisor + Phoenix.PubSub + Phoenix.Tracker + Telemetry) has been wired for. cosmp_router re-wire forward-substrate to sub-arc 1 sub-phase b and beyond; architectural target (hundreds to thousands of parallel COSMP operations per DMW for the workloads that need it) does not deliver at runtime until sub-arc 1 sub-phase b and beyond complete. **3-commit decomposition** per SYNTHESIS-SUB-PHASE-A-DECOMPOSITION: Commit 1 `[BEAM-DBGI-DMWWORKER-ADR]` docs-only at this commit; Commit 2 `[BEAM-DBGI-DMWWORKER-CODE]` substantive code at canonical-execution register substantively (NEW `apps/dbgi_supervisor/lib/dbgi_supervisor/dmw_worker.ex` + DynamicSupervisor wiring + Registry integration + Phoenix.Tracker integration + tier dispatch + tests) forward-substrate; Commit 3 `[BEAM-DBGI-DMWWORKER-CLOSURE]` docs-only closure cascade (ADR-0038 Status Proposed → Accepted + Post-Closure Implementation Lineage + this row CLOSED + catalog refreshes) forward-substrate. 7 substrate-build observations forward-queued at commit-body-only register substantively (D-AUTHORIZATION-PASTE-PROSE-VS-SCOPE-DISTINCTION + D-ADR-CATALOG-ENTRY-CHRONOLOGY-RESIDUAL + D-PUSH-RECONCILIATION-MID-COMMIT + D-OPERATOR-CORRECTION-EXTENSION-DISCIPLINE + D-OPERATOR-FRAMING-REALIGNMENT-DISCIPLINE + D-SUBSTANTIVE-CASCADE-PROSE-DRIFT + D-ADR-AMENDMENT-PATTERN-VARIANCE-DISCIPLINE NEW canonical at this commit); NOT promoted to ADR-0035 §9 numbered cluster. |
