...
| 12.5 Sub-box 2 Phase 1 (dual-control middleware) | IN FLIGHT | `b34c5cf` → … | The privileged-action dual-control gate per `COMPLIANCE_ARCHITECTURE_REVIEW.md` Tension 3 (consumes the generalized `requireDualControl` preHandler forward-queued from [D-2D-D10-7]). 10-commit arc — sub-phases A–H landed (`b34c5cf`→ this commit; H's hash cascaded into this row + the canonical-record §6 at sub-phase I per the post-commit-hash discipline); I `[SEC-CONTRIBUTOR-GOVERNANCE]` (ADR-0027 + RULE 20 + `onboarding-for-engineers.md`) + J `[SEC-BEAM-FORWARD-SUBSTRATE]` (ADR-0028) remaining. ADR-0026 (the bundle) landed at sub-phase H; operational companion `docs/architecture/dual-control-operations-canonical-record.md`. |
