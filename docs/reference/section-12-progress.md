...
| 12.5 Sub-box 2 Phase 1 (dual-control middleware) | IN FLIGHT | `b34c5cf` → `135fee0` → … | The privileged-action dual-control gate per `COMPLIANCE_ARCHITECTURE_REVIEW.md` Tension 3 (consumes the generalized `requireDualControl` preHandler forward-queued from [D-2D-D10-7]). 10-commit arc — sub-phases A–I landed (`b34c5cf` → `135fee0` [H] → this commit [I]; the `135fee0` H-hash backfilled into this row + the canonical-record §6 at this sub-phase I per the post-commit-hash discipline; I's own hash cascaded at sub-phase J); J `[SEC-BEAM-FORWARD-SUBSTRATE]` (ADR-0028 — the Elixir/BEAM coordination-layer commitment-to-ship) remaining. ADR-0026 (the dual-control bundle) landed at sub-phase H; ADR-0027 (contributor governance + RULE 20) landed at sub-phase I; operational companion `docs/architecture/dual-control-operations-canonical-record.md`. |
